...
describe("CodePath", () => {
	...
	describe("CodePath.HasPath - Basic Functionality", () => {
		test("construction and basic properties", () => {
			...
			const expectedCodePath = TsQIdent.of(
				library,
				...pathPart.parts.toArray(),
			);
			expect(hasPath.codePath.parts.toArray().map((p) => p.value)).toEqual(
				expectedCodePath.parts.toArray().map((p) => p.value),
			);
		});
		...
		test("get returns Some", () => {
			...
			if (isSome(result)) {
				...
				expect(
					result.value.codePathPart.parts.toArray().map((p) => p.value),
		...
		test("forceHasPath returns self", () => {
			...
			expect(result.codePathPart.parts.toArray().map((p) => p.value)).toEqual(
				hasPath.codePathPart.parts.toArray().map((p) => p.value),
			);
		});
	...
	describe("Edge Cases and Boundary Conditions", () => {
		...
		test("single character identifiers", () => {
			...
			expect(hasPath.codePathPart.parts.toArray().map((p) => p.value)).toEqual([
				"b",
			]);
			expect(hasPath.codePath.parts.toArray().map((p) => p.value)).toEqual([
				"a",
	...
	describe("Complex Real-World Scenarios", () => {
		test("nested module structure", () => {
			...
			const result = hasPath
				.add(TsIdent.simple("Props"))
				.add(TsIdent.simple("onClick"));

			expect(CodePath.isHasPath(result)).toBe(true);
			if (CodePath.isHasPath(result)) {
				expect(result.inLibrary.value).toBe("react");
				expect(result.codePathPart.parts.toArray().map((p) => p.value)).toEqual(
					["components", "Button", "Props", "onClick"],
				);
				expect(result.codePath.parts.toArray().map((p) => p.value)).toEqual([
					"react",
					"components",
					"Button",
					"Props",
	...
	describe("Type Safety and Polymorphism", () => {
		test("CodePath trait polymorphism", () => {
			...
			const hasPath: CodePath = CodePath.hasPath(
				TsIdent.simple("lib"),
		...
		test("HasCodePath trait integration", () => {
			...
			const newPath = CodePath.hasPath(
				TsIdent.simple("newLib"),
				TsQIdent.ofStrings("newModule"),
			);
			...
			if (CodePath.isHasPath(updated.codePath)) {
				...
				expect(
					updated.codePath.codePathPart.parts.toArray().map((p) => p.value),
				).toEqual(["newModule"]);
			}
		});
	});
	...
	describe("Interoperability with Other TypeScript Types", () => {
		test("with TsIdentModule", () => {
			const moduleIdent = TsIdent.module(some("scope"), [
				"module",
				"submodule",
			]);
			const library = TsIdent.simple("myLib");
	...
	describe("Error Handling and Robustness", () => {
		...
		test("immutability of operations", () => {
			...
			if (CodePath.isHasPath(added)) {
				expect(added.codePathPart.parts.toArray().map((p) => p.value)).toEqual([
					"original",
					"added",
				]);
			}
			...
			if (CodePath.isHasPath(replaced)) {
				expect(
					replaced.codePathPart.parts.toArray().map((p) => p.value),
				).toEqual(["replaced"]);
			}
		});
	});
