...
import { PackageJson, Json } from '@/internal/ts/PackageJson.js';
...
function mapHasLibrary(map: Map<TsIdentLibrary, any>, targetValue: string): boolean {
...
function mapGetLibrary<V>(map: Map<TsIdentLibrary, V>, targetValue: string): V | undefined {
...
describe("PackageJson", () => {
  describe("Basic Construction and Empty PackageJson", () => {
    ...
    test("manual construction with all fields", () => {
      ...
      const devDeps = new Map([[TsIdentLibrary.construct("@types/node"), "^18.0.0"]]);
      const peerDeps = new Map([[TsIdentLibrary.construct("react"), ">=16.0.0"]]);
      const files = IArray.apply("index.d.ts", "lib/");
      const dist = { tarball: "https://registry.npmjs.org/test/-/test-1.0.0.tgz" };
  ...
  describe("allLibs Method", () => {
    ...
    test("allLibs including dev dependencies", () => {
      const deps = new Map([[TsIdentLibrary.construct("lodash"), "^4.17.21"]]);
      const devDeps = new Map([[TsIdentLibrary.construct("typescript"), "^4.9.0"]]);
      const packageJson = new PackageJson(
        "1.0.0",
        deps,
        devDeps
    ...
    test("allLibs with all dependency types", () => {
      ...
      const sortedKeys = [...keys].sort((a, b) => a.value.localeCompare(b.value));
  ...
  describe("Edge Cases and Boundary Conditions", () => {
    ...
    test("PackageJson.Dist parsing", () => {
      ...
      if (!(result instanceof Error)) {
        ...
        expect(result.dist!.tarball).toBe("https://registry.npmjs.org/test/-/test-1.0.0.tgz");
      }
    });
  });
});