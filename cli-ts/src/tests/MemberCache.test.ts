...
describe("MemberCache Tests", () => {
	describe("MemberCache - Basic Functionality", () => {
		...
		it("should handle single named declaration", () => {
			...
			const cache = new TestMemberCache(
				IArray.fromArray<TsContainerOrDecl>([mockClass]),
			);
		...
		it("should handle single export declaration", () => {
			...
			const cache = new TestMemberCache(
				IArray.fromArray<TsContainerOrDecl>([mockExport]),
			);
		...
		it("should handle single import declaration", () => {
			...
			const cache = new TestMemberCache(
				IArray.fromArray<TsContainerOrDecl>([mockImport]),
			);
		...
		it("should handle local import does not make it a module", () => {
			...
			const cache = new TestMemberCache(
				IArray.fromArray<TsContainerOrDecl>([localImport]),
			);
	...
	describe("MemberCache - Modules Functionality", () => {
		...
		it("should handle single module", () => {
			...
			const cache = new TestMemberCache(
				IArray.fromArray<TsContainerOrDecl>([mockModule]),
			);
		...
		it("should handle multiple modules", () => {
			...
			const cache = new TestMemberCache(
				IArray.fromArray<TsContainerOrDecl>([mockModule1, mockModule2]),
			);
	...
	describe("MemberCache - Augmented Modules Functionality", () => {
		...
		it("should handle single augmented module", () => {
			...
			const cache = new TestMemberCache(
				IArray.fromArray<TsContainerOrDecl>([augmentedModule]),
			);
			...
			expect(cache.augmentedModulesMap.get(augmentedModule.name)?.head).toBe(
				augmentedModule,
			);
		});
		...
		it("should handle multiple augmented modules with same name", () => {
			...
			expect(cache.augmentedModulesMap.size).toBe(1);
			const sameNameAugmented = cache.augmentedModulesMap.get(
				augmentedModule1.name,
			);
	...
	describe("HasClassMembers - Basic Functionality", () => {
		...
		it("should handle multiple members with same name", () => {
			...
			const hasClassMembers = new TestHasClassMembers(
				IArray.fromArray([memberFunction1, memberFunction2] as TsMember[]),
			);

			expect(hasClassMembers.membersByName.size).toBe(1);
			expect(hasClassMembers.membersByName.has(memberFunction1.name)).toBe(
				true,
			);
			const sameNameMembers = hasClassMembers.membersByName.get(
				memberFunction1.name,
			);
		...
		it("should handle mixed member types", () => {
			...
			expect(hasClassMembers.membersByName.has(TsIdentConstructorIdent)).toBe(
				true,
			);
			expect(hasClassMembers.unnamed.isEmpty).toBe(true);
		});
		...
		it("should handle multiple members with different names", () => {
			...
			const hasClassMembers = new TestHasClassMembers(
				IArray.fromArray([memberFunction, memberProperty] as TsMember[]),
			);
			...
			expect(hasClassMembers.membersByName.get(memberProperty.name)?.head).toBe(
				memberProperty,
			);
			expect(hasClassMembers.unnamed.isEmpty).toBe(true);
		});
	...
	describe("MemberCache - Edge Cases and Error Conditions", () => {
		it("should handle large number of members", () => {
			const members = Array.from({ length: 100 }, (_, i) =>
				createMockClass(`Class${i}`),
			);
			const cache = new TestMemberCache(
				IArray.fromArray(members as TsContainerOrDecl[]),
			);
		...
		it("should handle mixed large collection", () => {
			...
			const exports = Array.from({ length: 25 }, (_, i) =>
				createMockExport(`Export${i}`),
			);
			const imports = Array.from({ length: 25 }, (_, i) =>
				createMockImport(`module${i}`),
			);
			const allMembers = [...classes, ...exports, ...imports];

			const cache = new TestMemberCache(
				IArray.fromArray(allMembers as TsContainerOrDecl[]),
			);
		...
		it("should handle complex nested scenarios", () => {
			...
			const augmentedGroup = cache.augmentedModulesMap.get(
				augmentedModule1.name,
			);
		...
		it("should maintain consistency across all computed properties", () => {
			...
			const totalPartitioned =
				cache.nameds.length +
				cache.exports.length +
				cache.imports.length +
				cache.unnamed.length;
			...
			expect(cache.augmentedModules.length).toBeLessThanOrEqual(
				cache.nameds.length,
			);
		});
	});
