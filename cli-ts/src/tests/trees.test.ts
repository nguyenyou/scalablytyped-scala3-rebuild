	...
	describe("TsParsedFile", () => {
		describe("construction", () => {
			...
			it("should detect non-standard library files", () => {
				const comments = Comments.empty();
				const directives = IArray.fromArray<Directive>([
		...
		describe("manipulation", () => {
			...
			it("should support withCodePath", () => {
				...
				const newCodePath = CodePath.hasPath(
					TsIdent.simple("test"),
					TsQIdent.empty(),
				);
				const newParsedFile = parsedFile.withCodePath(
					newCodePath,
				) as TsParsedFile;
	...
	describe("TsTypeParam", () => {
		describe("construction", () => {
			...
			it("should create a type parameter with upper bound", () => {
				...
				const upperBound = {
					_tag: "TsTypeRef",
					name: TsQIdent.of(TsIdent.simple("string")),
	...
	describe("TsFunParam", () => {
		...
		describe("equality", () => {
			...
			it("should consider parameters with different types unequal", () => {
				...
				const type2 = {
					_tag: "TsTypeRef",
					name: TsQIdent.of(TsIdent.simple("number")),
					tparams: IArray.Empty,
					asString: "number",
				} as any;
	...
	describe("TsFunSig", () => {
		describe("construction", () => {
			it("should create a simple function signature", () => {
				const param1 = TsFunParam.typed(TsIdent.simple("x"), {
					_tag: "TsTypeRef",
					asString: "number",
				} as any);
				const param2 = TsFunParam.typed(TsIdent.simple("y"), {
					_tag: "TsTypeRef",
					asString: "string",
				} as any);
			...
			it("should create a function signature with type parameters", () => {
				...
				const param = TsFunParam.typed(TsIdent.simple("value"), {
					_tag: "TsTypeRef",
					asString: "T",
				} as any);
				...
				const signature = TsFunSig.withTypeParams(
					tparams,
					params,
					some(returnType),
				);
	...
	describe("TsTypeIntersect", () => {
		describe("construction", () => {
			...
			it("should flatten nested intersection types", () => {
				...
				const outerTypes = IArray.fromArray<TsType>([
					innerIntersect,
					booleanType,
				]);
	...
	describe("TsMemberCall", () => {
		describe("construction", () => {
			it("should create a call signature member", () => {
				...
				const callMember = TsMemberCall.create(
					Comments.empty(),
					level,
					signature,
				);
	...
	describe("TsMemberCtor", () => {
		describe("construction", () => {
			it("should create a constructor signature member", () => {
				const signature = TsFunSig.noParams(some(TsTypeRef.string));
				const level = TsProtectionLevel.default();
				const ctorMember = TsMemberCtor.create(
					Comments.empty(),
					level,
					signature,
				);
	...
	describe("TsImported", () => {
		...
		describe("TsImportedDestructured", () => {
			...
			it("should identify destructured imports", () => {
				...
				const imported = TsImportedDestructured.simple(idents);
				expect(TsImportedDestructured.isImportedDestructured(imported)).toBe(
					true,
				);

				const notImported = { _tag: "SomethingElse", asString: "test" };
				expect(TsImportedDestructured.isImportedDestructured(notImported)).toBe(
					false,
				);
			});
		});
		...
		describe("visit", () => {
			it("should transform expressions recursively", () => {
				...
				const transformed = TsExpr.visit(original, (expr) => {
					if (
						TsExpr.isRef(expr) &&
						expr.value.asString === "TsQIdent(oldName)"
					) {
	...
	describe("TsEnumMember", () => {
		...
		describe("utilities", () => {
			...
			it("should get effective value of enum members", () => {
				...
				const numericMember = TsEnumMember.numeric(
					TsIdent.simple("Numeric"),
					42,
				);
...
describe("trees - Phase 11: Advanced Type Features", () => {
	describe("TsTypeConstructor", () => {
		describe("construction", () => {
			it("should create a concrete constructor type", () => {
				const signature = TsTypeFunction.create(
					TsFunSig.simple(IArray.Empty, some(TsTypeRef.string)),
				);
	...
	describe("TsTypeExtends", () => {
		...
		describe("type guards", () => {
			it("should identify extends types", () => {
				const extendsClause = TsTypeExtends.create(
					TsTypeRef.string,
					TsTypeRef.object,
				);
	...
	describe("string representation", () => {
		...
		it("should format type predicates", () => {
			const ident = TsIdent.simple("value");
			const predicate = TsTypeIs.create(ident, TsTypeRef.string);
			expect(predicate.asString).toBe(
				"TsTypeIs(value is TsTypeRef(TsQIdent(string)))",
			);
		});
		...
		it("should format assertions", () => {
			...
			expect(assertion.asString).toBe(
				"TsTypeAsserts(asserts value is TsTypeRef(TsQIdent(string)))",
			);
		});
		...
		it("should format tuple elements", () => {
			...
			expect(element.asString).toBe(
				"TsTupleElement(name: TsTypeRef(TsQIdent(string)))",
			);
		});
		...
		it("should format repeated types", () => {
			const repeated = TsTypeRepeated.create(TsTypeRef.string);
			expect(repeated.asString).toBe(
				"TsTypeRepeated(...TsTypeRef(TsQIdent(string)))",
			);
		});

		it("should format keyof types", () => {
			const keyof = TsTypeKeyOf.create(TsTypeRef.string);
			expect(keyof.asString).toBe(
				"TsTypeKeyOf(keyof TsTypeRef(TsQIdent(string)))",
			);
		});

		it("should format lookup types", () => {
			const lookup = TsTypeLookup.create(TsTypeRef.string, TsTypeRef.number);
			expect(lookup.asString).toBe(
				"TsTypeLookup(TsTypeRef(TsQIdent(string))[TsTypeRef(TsQIdent(number))])",
			);
		});
		...
		it("should format conditional types", () => {
			...
			expect(conditional.asString).toBe(
				"TsTypeConditional(TsTypeRef(TsQIdent(string)) ? TsTypeRef(TsQIdent(number)) : TsTypeRef(TsQIdent(boolean)))",
			);
		});
		...
		it("should format extends clauses", () => {
			...
			expect(extendsClause.asString).toBe(
				"TsTypeExtends(TsTypeRef(TsQIdent(string)) extends TsTypeRef(TsQIdent(object)))",
			);
		});
	...
	describe("TsMemberTypeMapped", () => {
		describe("construction", () => {
			it("should create a simple mapped type member", () => {
				...
				const to = TsTypeLookup.create(
					TsTypeRef.string,
					TsTypeRef.fromIdent(key),
				);
			...
			it("should create a mapped type member with key remapping", () => {
				const key = TsIdent.simple("K");
				const from = TsTypeKeyOf.create(TsTypeRef.string);
				const as = TsTypeTemplateLiteral.simple(
					"prefix_",
					TsTypeRef.fromIdent(key),
					"_suffix",
				);
	...
	describe("TsTemplatePartLiteral", () => {
		...
		describe("type guards", () => {
			it("should identify literal template parts", () => {
				...
				expect(TsTemplatePartLiteral.isTemplatePartLiteral(notPart)).toBe(
					false,
				);
			});
		});
	...
	describe("TsTypeTemplateLiteral", () => {
		describe("construction", () => {
			...
			it("should create a simple template with interpolation", () => {
				const template = TsTypeTemplateLiteral.simple(
					"prefix_",
					TsTypeRef.string,
					"_suffix",
				);
		...
		describe("type guards", () => {
			it("should identify template literal types", () => {
				...
				expect(TsTypeTemplateLiteral.isTypeTemplateLiteral(template)).toBe(
					true,
				);

				const notTemplate = TsTypeRef.string;
				expect(TsTypeTemplateLiteral.isTypeTemplateLiteral(notTemplate)).toBe(
					false,
				);
			});
		});
	...
	describe("TsType utilities", () => {
		describe("type mapping detection", () => {
			...
			it("should not detect non-mapped types as mapped", () => {
				const prop = TsMemberProperty.typed(
					TsIdent.simple("prop"),
					TsTypeRef.string,
				);
		...
		describe("mapped type operations", () => {
			it("should detect mapped types", () => {
				...
				const notMappedType = TsTypeObject.create(
					Comments.empty(),
					IArray.Empty,
				);
...
describe("trees - Phase 13: Declaration System Completion", () => {
	describe("TsDeclEnum", () => {
		...
		describe("type guards", () => {
			it("should identify enum declarations", () => {
				const enumDecl = TsDeclEnum.simple(
					TsIdent.simple("Test"),
					IArray.Empty,
				);
	...
	describe("TsDeclTypeAlias", () => {
		describe("construction", () => {
			it("should create a simple type alias declaration", () => {
				...
				const alias = TsTypeUnion.create(
		...
		describe("type guards", () => {
			it("should identify type alias declarations", () => {
				const typeAlias = TsDeclTypeAlias.simple(
					TsIdent.simple("Test"),
					TsTypeRef.string,
				);
	...
	describe("TsDeclVar", () => {
		...
		describe("type guards", () => {
			it("should identify variable declarations", () => {
				const varDecl = TsDeclVar.simple(
					TsIdent.simple("test"),
					TsTypeRef.string,
				);
	...
	describe("TsDeclFunction", () => {
		...
		describe("type guards", () => {
			it("should identify function declarations", () => {
				const funcDecl = TsDeclFunction.noParams(
					TsIdent.simple("test"),
					TsTypeRef.void,
				);
	...
	describe("string representation", () => {
		it("should format enum declarations", () => {
			...
			const constEnum = TsDeclEnum.const(
				TsIdent.simple("Status"),
				IArray.Empty,
			);
			expect(constEnum.asString).toBe("TsDeclEnum(Status)");

			const declaredEnum = TsDeclEnum.declared(
				TsIdent.simple("Direction"),
				IArray.Empty,
			);
		...
		it("should format type alias declarations", () => {
			const typeAlias = TsDeclTypeAlias.simple(
				TsIdent.simple("StringOrNumber"),
				TsTypeRef.string,
			);
			expect(typeAlias.asString).toBe("TsDeclTypeAlias(StringOrNumber)");

			const declaredAlias = TsDeclTypeAlias.declared(
				TsIdent.simple("ExternalType"),
				TsTypeRef.string,
			);
		...
		it("should format variable declarations", () => {
			const varDecl = TsDeclVar.simple(
				TsIdent.simple("myVar"),
				TsTypeRef.string,
			);
			expect(varDecl.asString).toBe("TsDeclVar(myVar)");

			const constDecl = TsDeclVar.const(
				TsIdent.simple("myConst"),
				TsTypeRef.number,
			);
			expect(constDecl.asString).toBe("TsDeclVar(myConst)");

			const declaredVar = TsDeclVar.declared(
				TsIdent.simple("globalVar"),
				TsTypeRef.string,
			);
...
describe("trees - Phase 14: Type System Optimizations", () => {
	describe("TsTypeIntersect.simplified", () => {
		...
		it("should combine multiple object types into one", () => {
			const prop1 = TsMemberProperty.typed(
				TsIdent.simple("prop1"),
				TsTypeRef.string,
			);
			const prop2 = TsMemberProperty.typed(
				TsIdent.simple("prop2"),
				TsTypeRef.number,
			);

			const obj1 = TsTypeObject.withMembers(
				IArray.fromArray([prop1] as TsMember[]),
			);
			const obj2 = TsTypeObject.withMembers(
				IArray.fromArray([prop2] as TsMember[]),
			);
		...
		it("should handle mapped types correctly", () => {
			...
			const mapped = TsMemberTypeMapped.simple(key, from, to);
			const mappedObj = TsTypeObject.withMembers(
				IArray.fromArray([mapped] as TsMember[]),
			);
			...
			const normalObj = TsTypeObject.withMembers(
				IArray.fromArray([prop] as TsMember[]),
			);
		...
		it("should flatten nested intersections", () => {
			...
			const nested = TsTypeIntersect.create(
				IArray.fromArray<TsType>([type2, type3]),
			);
			const result = TsTypeIntersect.simplified(
				IArray.fromArray<TsType>([type1, nested]),
			);
		...
		it("should remove duplicates", () => {
			const stringType = TsTypeRef.string;
			const result = TsTypeIntersect.simplified(
				IArray.fromArray<TsType>([stringType, stringType]),
			);
			expect(result).toBe(stringType);
		});
	});
	...
	describe("TsTypeUnion.simplified", () => {
		...
		it("should flatten nested unions", () => {
			...
			const nested = TsTypeUnion.create(
				IArray.fromArray<TsType>([type2, type3]),
			);
			const result = TsTypeUnion.simplified(
				IArray.fromArray<TsType>([type1, nested]),
	...
	describe("TsType utility functions", () => {
		...
		it("should return none for non-mapped types", () => {
			...
			const objType = TsTypeObject.withMembers(
				IArray.fromArray([prop] as TsMember[]),
			);
