...
import { LibraryResolver, LibraryResolverRes } from '../internal/importer/LibraryResolver.js';
import { TsIdentLibrary, TsIdentModule } from '../internal/ts/trees.js';
import { LibTsSource } from '../internal/importer/LibTsSource.js';
import { InFile, InFolder, filesSync } from '../internal/files.js';
import { IArray } from '../internal/IArray.js';
import { ResolvedModuleLocal, ResolvedModuleNotLocal } from '../internal/importer/ResolvedModule.js';
...
function withTempDir<T>(testName: string, test: (tempDir: string) => T): T {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `library-resolver-test-${testName}-`));
  try {
    return test(tempDir);
  } finally {
...
function createMockStdLib(tempDir: string): LibTsSource.StdLibSource {
  ...
  createTestFile(path.join(stdLibDir, 'lib.d.ts'), 'declare var console: Console;');
  createTestFile(path.join(stdLibDir, 'lib.es6.d.ts'), 'interface Promise<T> {}');
  ...
  return new LibTsSource.StdLibSource(stdLibFolder, stdLibFiles, TsIdentLibrary.construct('std'));
}

function createMockLibrary(tempDir: string, libName: string, hasIndexDts: boolean = true): LibTsSource.FromFolder {
  const libDir = path.join(tempDir, 'node_modules', libName);
  createTestDir(libDir);
  
  if (hasIndexDts) {
    createTestFile(path.join(libDir, 'index.d.ts'), `export declare const ${libName}: any;`);
  }
  createTestFile(path.join(libDir, 'package.json'), `{"name": "${libName}", "version": "1.0.0"}`);
  
  return new LibTsSource.FromFolder(new InFolder(libDir), TsIdentLibrary.construct(libName));
}

function createMockScopedLibrary(tempDir: string, scope: string, name: string): LibTsSource.FromFolder {
  const libDir = path.join(tempDir, 'node_modules', `@${scope}`, name);
  createTestDir(libDir);
  createTestFile(path.join(libDir, 'index.d.ts'), `export declare const ${name}: any;`);
  createTestFile(path.join(libDir, 'package.json'), `{"name": "@${scope}/${name}", "version": "1.0.0"}`);
  
  return new LibTsSource.FromFolder(new InFolder(libDir), TsIdentLibrary.construct(`@${scope}/${name}`));
}
...
describe('LibraryResolver', () => {
  describe('Construction', () => {
    ...
    test('should initialize with multiple sources', () => {
      withTempDir('multi-init', (tempDir) => {
        ...
        const lodashResult = resolver.library(TsIdentLibrary.construct('lodash'));
    ...
    test('should handle duplicate library names by taking first', () => {
      withTempDir('duplicate-names', (tempDir) => {
        ...
        const lodash1 = createMockLibrary(path.join(tempDir, 'location1'), 'lodash');
        const lodash2 = createMockLibrary(path.join(tempDir, 'location2'), 'lodash');
  ...
  describe('Library Resolution', () => {
    ...
    test('should find existing scoped library', () => {
      withTempDir('find-scoped', (tempDir) => {
        ...
        const result = resolver.library(TsIdentLibrary.construct('@angular/core'));
        expect(result.type).toBe('Found');
        if (result.type === 'Found') {
    ...
    test('should return NotAvailable for non-existent library', () => {
      withTempDir('not-found', (tempDir) => {
        ...
        const result = resolver.library(TsIdentLibrary.construct('non-existent'));
  ...
  describe('Res ADT Behavior', () => {
    ...
    test('Ignored should convert to None in toOption', () => {
      const ignored = LibraryResolverRes.Ignored(TsIdentLibrary.construct('test'));
    ...
    test('NotAvailable should convert to None in toOption', () => {
      const notAvailable = LibraryResolverRes.NotAvailable(TsIdentLibrary.construct('test'));
    ...
    test('Found should map correctly', () => {
      ...
      const mapped = LibraryResolverRes.map((s: string) => s.toUpperCase())(found);
    ...
    test('NotAvailable should preserve type in map', () => {
      const notAvailable = LibraryResolverRes.NotAvailable(TsIdentLibrary.construct('test'));
      const mapped = LibraryResolverRes.map((s: any) => s.toString())(notAvailable);
  ...
  describe('Module Resolution', () => {
    test('should resolve local path modules', () => {
      withTempDir('local-modules', (tempDir) => {
        ...
        createTestFile(path.join(sourceFolder.path, 'utils.ts'), "export const util = 'test';");
  ...
  describe('File Resolution', () => {
    test('should find exact file match', () => {
      withTempDir('exact-file', (tempDir) => {
        ...
        createTestFile(path.join(tempDir, 'test.ts'), "export const test = 'value';");
    ...
    test('should find file with .d.ts extension added', () => {
      withTempDir('dts-extension', (tempDir) => {
        const folder = new InFolder(tempDir);
        createTestFile(path.join(tempDir, 'test.d.ts'), 'declare const test: string;');
    ...
    test('should find index.d.ts in subdirectory', () => {
      withTempDir('index-subdir', (tempDir) => {
        ...
        createTestFile(path.join(tempDir, 'test', 'index.d.ts'), 'declare const test: string;');
  ...
  describe('Module Name Generation', () => {
    test('should generate module names for simple library', () => {
      withTempDir('simple-module-names', (tempDir) => {
        ...
        createTestDir(libDir);
        createTestFile(path.join(libDir, 'index.d.ts'), 'export declare const _: any;');
        createTestFile(path.join(libDir, 'utils.d.ts'), 'export declare const utils: any;');

        const source = new LibTsSource.FromFolder(new InFolder(libDir), TsIdentLibrary.construct('lodash'));
    ...
    test('should generate module names for scoped library', () => {
      withTempDir('scoped-module-names', (tempDir) => {
        ...
        createTestDir(libDir);
        createTestFile(path.join(libDir, 'index.d.ts'), 'export declare const core: any;');
        createTestFile(path.join(libDir, 'testing.d.ts'), 'export declare const testing: any;');

        const source = new LibTsSource.FromFolder(new InFolder(libDir), TsIdentLibrary.construct('@angular/core'));
    ...
    test('should handle lib/es parallel directory mapping', () => {
      withTempDir('parallel-dirs', (tempDir) => {
        ...
        createTestDir(libDir);
        createTestFile(path.join(libDir, 'lib', 'button.d.ts'), 'export declare const Button: any;');

        const source = new LibTsSource.FromFolder(new InFolder(libDir), TsIdentLibrary.construct('antd'));
        const file = new InFile(path.join(libDir, 'lib', 'button.d.ts'));

        const moduleNames = LibraryResolver.moduleNameFor(source, file);

        // Should generate both lib and es versions
        const hasLibVersion = moduleNames.toArray().some(m => m.fragments.includes('lib'));
        const hasEsVersion = moduleNames.toArray().some(m => m.fragments.includes('es'));
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    test('should handle special characters in library names', () => {
      withTempDir('special-chars', (tempDir) => {
        const stdLib = createMockStdLib(tempDir);
        const specialLib = createMockLibrary(tempDir, 'lib-with-dashes_and_underscores');
        const allSources = IArray.fromArray([specialLib]);
        const ignored = new Set<TsIdentLibrary>();

        const resolver = new LibraryResolver(stdLib, allSources, ignored);

        const result = resolver.library(TsIdentLibrary.construct('lib-with-dashes_and_underscores'));
        expect(result.type).toBe('Found');
        if (result.type === 'Found') {
    ...
    test('should handle malformed scoped library names', () => {
      withTempDir('malformed-scoped', (tempDir) => {
        ...
        const malformedNames = ['@', '@scope', '@scope/', '@/name', '@@scope/name'];
