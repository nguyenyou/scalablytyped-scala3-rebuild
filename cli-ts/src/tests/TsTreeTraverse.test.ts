...
describe("TsTreeTraverse Tests", () => {
	...
	describe("TsTreeTraverse - Basic Functionality", () => {
		...
		test("collectIArray with multiple trees", () => {
			...
			const trees = createIArray([
				class1 as TsTree,
				class2 as TsTree,
				interface1 as TsTree,
			]);
		...
		test("collect extracts all matching nodes", () => {
			...
			const members = createIArray([
				memberProp as TsMember,
				memberFunc as TsMember,
			]);
	...
	describe("TsTreeTraverse - Nested Structures", () => {
		...
		test("traverse module with nested declarations", () => {
			...
			const members = createIArray([
				nestedClass as TsContainerOrDecl,
				nestedInterface as TsContainerOrDecl,
				nestedVar as TsContainerOrDecl,
			]);
	...
	describe("TsTreeTraverse - Complex Hierarchies", () => {
		...
		test("parsed file with multiple containers", () => {
			...
			const members = createIArray([
				class1 as TsContainerOrDecl,
				interface1 as TsContainerOrDecl,
				module1 as TsContainerOrDecl,
			]);
		...
		test("nested modules with deep hierarchy", () => {
			...
			const innerModule = createMockModule(
				"InnerModule",
				createIArray([innerVar as TsContainerOrDecl]),
			);
			const middleModule = createMockModule(
				"MiddleModule",
				createIArray([innerModule as TsContainerOrDecl]),
			);
			const outerModule = createMockModule(
				"OuterModule",
				createIArray([middleModule as TsContainerOrDecl]),
			);
	...
	describe("TsTreeTraverse - Advanced Extraction Scenarios", () => {
		test("extract specific node types with conditions", () => {
			...
			const members = createIArray([
				memberProp1 as TsMember,
				memberProp2 as TsMember,
				memberFunc as TsMember,
			]);
			const mockClass = createMockClass("TestClass", members);
		...
		test("extract nested identifiers", () => {
			...
			const mockClass = createMockClass(
				"TestClass",
				createIArray([memberProp as TsMember]),
			);
	...
	describe("TsTreeTraverse - Performance and Stress Tests", () => {
		test("large tree traversal", () => {
			...
			expect(result.toArray().every((name) => name.startsWith("prop"))).toBe(
				true,
			);
		});
	...
	describe("TsTreeTraverse - Integration with collectIArray", () => {
		test("collect from multiple complex trees", () => {
			const class1 = createMockClass(
				"Class1",
				createIArray([createMemberProperty("prop1") as TsMember]),
			);
			const class2 = createMockClass(
				"Class2",
				createIArray([createMemberProperty("prop2") as TsMember]),
			);
			const interface1 = createMockInterface(
				"Interface1",
				createIArray([createMemberFunction("method1") as TsMember]),
			);
			const trees = createIArray([
				class1 as TsTree,
				class2 as TsTree,
		...
		test("collectIArray with mixed extraction criteria", () => {
			...
			const typeResults = TsTreeTraverse.collectIArray(
				trees,
				(tree: TsTree) => {
					if (tree._tag === "TsTypeRef") {
						return (tree as TsTypeRef).name.parts.apply(0).value;
			...
			const classResults = TsTreeTraverse.collectIArray(
				trees,
				(tree: TsTree) => {
					if (tree._tag === "TsDeclClass") {
						return (tree as TsDeclClass).name.value;
		...
		test("collectIArray performance with many trees", () => {
			...
			expect(result.toArray().every((name) => name.startsWith("Type"))).toBe(
				true,
			);
		});
		...
		test("collectIArray functional equivalence with collect", () => {
			...
			const collectResult = TsTreeTraverse.collect(
				mockClass,
				(tree: TsTree) => {
					if (tree._tag === "TsIdentSimple") {
						return (tree as TsIdentSimple).value;
			...
			const collectIArrayResult = TsTreeTraverse.collectIArray(
				createIArray([mockClass as TsTree]),
				(tree: TsTree) => {
					if (tree._tag === "TsIdentSimple") {
						return (tree as TsIdentSimple).value;
					}
			...
			expect(collectResult.toArray().sort()).toEqual(
				collectIArrayResult.toArray().sort(),
			);
		});
	});
