...
const dependencyPhase =
	(deps: Set<TestId>): Phase<TestId, TestValue, TestValue> =>
	(
		id: TestId,
		value: TestValue,
		...
		if (deps.size > 0 && !deps.has(id)) {
			...
			if (depsResult._tag === "Ok") {
				...
				return PhaseRes.Ok({
					data: `${value.data}-with-${depCount}-deps`,
					step: value.step + 1,
				});
			} else {
	...
	describe("PhaseRunner caching behavior", () => {
		test("should cache phase results across executions", () => {
			...
			const countingPhase: Phase<TestId, TestId, TestValue> = (
				...
				computeCount += 1;
				return PhaseRes.Ok({
					data: `computed-${value}-${computeCount}`,
					step: computeCount,
				});
			};

			const phase = RecPhase.apply<TestId>().next(
				countingPhase,
				"counting-phase",
			);
			...
			// First execution
			const runner1 = PhaseRunner.apply(
				phase,
				getLogger,
				listener,
				Formatters.string(),
			...
			const runner2 = PhaseRunner.apply(
				phase,
				getLogger,
				listener,
				Formatters.string(),
	...
	describe("Complex phase pipelines", () => {
		test("should handle mixed success/failure scenarios", () => {
			const conditionalPhase =
				(
					shouldSucceed: (id: TestId) => boolean,
				): Phase<TestId, TestValue, TestValue> =>
				(
					id: TestId,
					value: TestValue,
					_getDeps: GetDeps<TestId, TestValue>,
					...
					if (shouldSucceed(id)) {
						return PhaseRes.Ok({
							data: `${value.data}-conditional`,
							step: value.step + 1,
						});
					} else {
						return PhaseRes.Failure(
							new Map([[id, right("conditional failure")]]),
						);
					}
				};
			...
			const phase = RecPhase.apply<TestId>()
				...
				.next(
					conditionalPhase((id) => id.includes("success")),
					"conditional",
				);
		...
		test("should handle complex dependency chains", () => {
			const multiDependencyPhase =
				...
				(
					...
					if (deps.size > 0 && !deps.has(id)) {
						const depsResult = getDeps(SortedSet.from(deps));
						if (depsResult._tag === "Ok") {
							const depData = Array.from(depsResult.value.values())
								.map((v) => v.data)
								.join(",");
							return PhaseRes.Ok({
			...
			const phase = RecPhase.apply<TestId>()
				...
				.next(
					multiDependencyPhase(new Set(["dep1", "dep2", "dep3"])),
					"multi-deps",
				);
