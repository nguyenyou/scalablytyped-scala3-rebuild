...
// Helper functions
const enrichDataWith = (
	data: ParsedData,
	deps: SortedMap<string, any>,
): EnrichedData => ({
...
describe("Documentation Examples - TypeScript", () => {
	describe("Simple Examples", () => {
		test("Simple single-phase example from README", () => {
			...
			const pipeline = RecPhase.apply<string>().next(
				upperCasePhase,
				"uppercase",
			);
	...
	describe("Circular Dependency Examples", () => {
		test("Circular dependency detection from README", () => {
			...
			const circularResult = PhaseRunner.go(
				pipeline,
				"test",
				["test"],
				getLogger,
	...
	describe("Advanced Features Examples", () => {
		test("Custom listener example from README", () => {
			...
			const startedEvents = customListener.events.filter(
				(e) => e.event._tag === "Started",
			);
			const successEvents = customListener.events.filter(
		...
		test("Conditional phase execution from README", () => {
			...
			const conditionalPipeline = RecPhase.apply<string>()
				.next(parsePhase, "parse")
				.nextOpt(enableValidation ? some(validatePhase) : none, "validate")
				.next(transformPhase, "transform");
			...
			const disabledRunner = PhaseRunner.apply(
				disabledValidationPipeline,
				getLogger,
				new CollectingPhaseListener(),
				Formatters.string(),
				Orderings.string(),
	...
	describe("Testing Examples from README", () => {
		test("Unit testing phases example from README", () => {
			...
			const mockGetDeps: GetDeps<string, string> = () =>
				PhaseRes.Ok(new SortedMap<string, string>());
			const result = upperCasePhase(
				"test",
				"hello",
				mockGetDeps,
				false,
		...
		test("Dependency testing example from README", () => {
			...
			const mockGetDeps: GetDeps<string, string> = () =>
				PhaseRes.Failure(
					new Map<string, Either<Error, string>>([
						["dep1", right("Not found")],
					]),
				);
			const result = dependencyPhase(
				"test",
				"input",
				mockGetDeps,
				false,
