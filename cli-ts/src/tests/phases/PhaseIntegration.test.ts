...
import { PhaseRunner, Formatters, Orderings } from '../../internal/phases/PhaseRunner';
import { RecPhase } from '../../internal/phases/RecPhase';
import { PhaseRes } from '../../internal/phases/PhaseRes';
import { PhaseListener, PhaseEvent, CollectingPhaseListener } from '../../internal/phases/PhaseListener';
...
const createFileData = (path: string, content: string, deps: FileId[] = []): FileData => ({
  path,
  content,
  processed: false,
...
const createProcessedFile = (id: FileId, originalPath: string, content: string): ProcessedFile => ({
  id,
  originalPath,
...
const parsePhase: Phase<FileId, FileId, FileData> =
  (id: FileId, value: FileId, getDeps: GetDeps<FileId, FileData>, isCircular: IsCircular, logger: Logger<void>) => {
...
const validatePhase: Phase<FileId, FileData, FileData> =
  (id: FileId, value: FileData, getDeps: GetDeps<FileId, FileData>, isCircular: IsCircular, logger: Logger<void>) => {
    // Simulate validation
    const errors: string[] = [];
...
const transformPhase: Phase<FileId, FileData, ProcessedFile> =
  (id: FileId, value: FileData, getDeps: GetDeps<FileId, ProcessedFile>, isCircular: IsCircular, logger: Logger<void>) => {
    if (!value.processed) {
      return PhaseRes.Failure(new Map([[id, right('File not validated')]]));
    }
    
    const processed = createProcessedFile(id, value.path, value.content.toUpperCase());
    processed.dependencyCount = value.dependencies.size;
    processed.processingSteps = ['parsed', 'validated', 'dependencies-resolved', 'transformed'];
    
    return PhaseRes.Ok(processed);
  };

const optimizePhase: Phase<FileId, ProcessedFile, ProcessedFile> =
  (id: FileId, value: ProcessedFile, getDeps: GetDeps<FileId, ProcessedFile>, isCircular: IsCircular, logger: Logger<void>) => {
    return PhaseRes.Ok({
      ...value,
...
describe('Phase Integration Tests', () => {
  describe('End-to-end pipeline execution', () => {
    test('should execute complete file processing pipeline', () => {
      ...
      if (result._tag === 'Ok') {
        ...
        expect(result.value.processedContent).toContain('PARSED CONTENT FOR MAIN.TS');
        expect(result.value.dependencyCount).toBe(2); // utils.ts and types.ts
        expect(result.value.processingSteps).toEqual([
          'parsed', 'validated', 'dependencies-resolved', 'transformed', 'optimized'
        ]);
      }
    ...
    test('should handle files with no dependencies', () => {
      ...
      if (result._tag === 'Ok') {
        ...
        expect(result.value.processedContent).toContain('PARSED CONTENT FOR STANDALONE.TS');
      }
    });
  });
    ...
    test('should handle circular dependencies', () => {
      ...
      if (result._tag === 'Failure') {
        ...
        expect(errors.some(error => 
          error._tag === 'Right' && error.right.includes('Circular dependency')
        )).toBe(true);
      }
    });
  });
  ...
  describe('Mixed success/failure/ignore scenarios', () => {
      ...
      const conditionalParsePhase: Phase<FileId, FileId, FileData> =
        (id: FileId, value: FileId, getDeps: GetDeps<FileId, FileData>, isCircular: IsCircular, logger: Logger<void>) => {
          if (value.includes('error')) {
            ...
            const content = `parsed content for ${value}`;
            const deps = value.includes('main') ? ['dep1.ts', 'error-dep.ts', 'ignore-dep.ts'] : [];
            return PhaseRes.Ok(createFileData(value, content, deps));
          }
        };
    ...
    test('should propagate failures through pipeline', () => {
      ...
      const transformSuccessEvents = transformEvents.filter(e => e.event._tag === 'Success');
