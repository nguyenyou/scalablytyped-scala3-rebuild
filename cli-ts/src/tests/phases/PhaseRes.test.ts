...
import { PhaseRes, map, flatMap, forEach, isOk, isIgnore, isFailure } from '@/internal/phases';
...
describe('PhaseRes', () => {
  describe('PhaseRes.Ok', () => {
    ...
    test('map should transform value', () => {
      ...
      const mapped = map<TestId, TestValue, string>(v => v.data.toUpperCase())(original);
    ...
    test('flatMap should propagate failures', () => {
      const original = PhaseRes.Ok<TestId, TestValue>({ data: 'test' });
      const errors = new Map<TestId, Either<Error, string>>([['id1', right('error')]]);
    ...
    test('forEach should perform side effect', () => {
      const original = PhaseRes.Ok<TestId, TestValue>({ data: 'test' });
      let sideEffect = '';
      const result = forEach<TestId, TestValue>(v => { sideEffect = v.data; })(original);
