...
import { RecPhase, RecPhaseInitial, RecPhaseNext } from '../../internal/phases/RecPhase';
...
const identityPhase = <Id, T>(): Phase<Id, T, T> => 
  (id: Id, value: T, getDeps: GetDeps<Id, T>, isCircular: IsCircular, logger: Logger<void>) =>
    PhaseRes.Ok(value);
...
describe('RecPhase', () => {
  ...
  describe('RecPhase.Next', () => {
    test('should chain multiple phases', () => {
      ...
      const phase2 = phase1.next(transformPhase(s => s.toUpperCase()), 'uppercase');
      const phase3 = phase2.next(transformPhase(s => s + '!'), 'exclamation');
    ...
    test('should support nextOpt with Some', () => {
      ...
      const phase1 = initial.next(toTestValuePhase, 'to-testvalue');
      const phase2 = phase1.nextOpt(some(transformPhase(s => s.toUpperCase())), 'optional-uppercase');
  ...
  describe('RecPhase composition patterns', () => {
    test('should support complex phase pipelines', () => {
      ...
      const pipeline = RecPhase.apply<TestId>()
        ...
        .next(transformPhase(s => s.toLowerCase()), 'lowercase')
        .next(transformPhase(s => s.charAt(0).toUpperCase() + s.slice(1)), 'capitalize')
        .nextOpt(some(transformPhase(s => s + ' World')), 'greeting')
        .nextOpt(none, 'skipped-step')
        .next(transformPhase(s => s.replace(' ', '_')), 'underscore');
  ...
  describe('RecPhase edge cases', () => {
    ...
    test('should support deeply nested phase chains', () => {
      ...
      // Chain many phases
      for (let i = 0; i < 10; i++) {
        current = current.next(transformPhase(s => s + i), `phase-${i}`);
      }
