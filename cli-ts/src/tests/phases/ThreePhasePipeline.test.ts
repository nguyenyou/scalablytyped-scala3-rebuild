...
import { isOk, isFailure, isIgnore } from '../../internal/phases/PhaseRes';
...
describe('Three-Phase Pipeline Integration', () => {
  test('should execute Phase1 → Phase2 → PhaseFlavour pipeline successfully', () => {
    ...
    const getLogger = (id: LibTsSource): Logger<void> => ({
      info: (msg: string) => console.log(`[${id.libName.value}] INFO: ${msg}`),
      warn: (msg: string) => console.log(`[${id.libName.value}] WARN: ${msg}`),
      error: (msg: string) => console.log(`[${id.libName.value}] ERROR: ${msg}`),
      debug: (msg: string) => console.log(`[${id.libName.value}] DEBUG: ${msg}`),
    ...
    if (isOk(result)) {
      ...
      console.log('Pipeline ignored the source (expected for some configurations)');
    } else {
