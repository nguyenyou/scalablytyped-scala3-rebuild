...
describe("FlattenTrees Tests", () => {
	...
	function createMockCodePath(path: string): CodePath {
		return CodePath.hasPath(
			createSimpleIdent(path),
			TsQIdent.of(createSimpleIdent(path)),
		);
	}
	...
	function createMockGlobal(
		...
		return TsGlobal.create(comments, declared, members, codePath);
	}
	...
	describe("FlattenTrees - apply methods", () => {
		test("single file processing", () => {
			...
			expect((result.members.apply(0) as TsDeclClass).name.value).toBe(
				"TestClass",
			);
		});

		test("multiple file merging", () => {
			const file1 = createMockParsedFile(
				"file1",
				Comments.empty(),
				IArray.Empty,
				IArray.fromArray<TsContainerOrDecl>([createMockClass("Class1")]),
			);
			const file2 = createMockParsedFile(
				"file2",
				Comments.empty(),
				IArray.Empty,
			...
			const classNames = result.members
				.map((m) => (m as TsDeclClass).name.value)
				.toSet();
	...
	describe("FlattenTrees - file merging", () => {
		...
		test("merge files with overlapping members", () => {
			const class1 = createMockClass(
				...
				undefined,
				IArray.Empty,
				IArray.fromArray<TsMember>([createMockProperty("prop1")]),
			);
			const class2 = createMockClass(
				"TestClass",
				Comments.empty(),
				false,
				false,
				...
				undefined,
				IArray.Empty,
				IArray.fromArray<TsMember>([createMockProperty("prop2")]),
			);
			const file1 = createMockParsedFile(
				"file1",
				Comments.empty(),
				IArray.Empty,
				IArray.fromArray<TsContainerOrDecl>([class1]),
			);
			const file2 = createMockParsedFile(
				"file2",
				Comments.empty(),
				IArray.Empty,
				IArray.fromArray<TsContainerOrDecl>([class2]),
			);
	...
	describe("FlattenTrees - member merging", () => {
		test("merge named and unnamed members", () => {
			...
			const hasNamedClass = result.exists(
				(m) =>
					TsDeclClass.isClass(m) &&
					(m as TsDeclClass).name.value === "NamedClass",
			);
		...
		test("merge TsGlobal members", () => {
			const global1 = createMockGlobal(
				Comments.empty(),
				false,
				IArray.fromArray<TsContainerOrDecl>([createMockClass("Class1")]),
			);
			const global2 = createMockGlobal(
				Comments.empty(),
				false,
				IArray.fromArray<TsContainerOrDecl>([createMockClass("Class2")]),
			);
		...
		test("preserve non-global unnamed members", () => {
			...
			const hasClass1 = result.exists(
				(m) =>
					TsDeclClass.isClass(m) && (m as TsDeclClass).name.value === "Class1",
			);
			const hasClass2 = result.exists(
				(m) =>
					TsDeclClass.isClass(m) && (m as TsDeclClass).name.value === "Class2",
			);
	...
	describe("FlattenTrees - named member merging", () => {
		test("merge namespaces with same name", () => {
			const ns1 = createMockNamespace(
				"TestNS",
				Comments.empty(),
				false,
				IArray.fromArray<TsContainerOrDecl>([createMockClass("Class1")]),
			);
			const ns2 = createMockNamespace(
				"TestNS",
				Comments.empty(),
				false,
				IArray.fromArray<TsContainerOrDecl>([createMockClass("Class2")]),
		...
		test("merge interfaces with same name", () => {
			const iface1 = createMockInterface(
				"TestInterface",
				Comments.empty(),
				false,
				IArray.Empty,
			...
			const iface2 = createMockInterface(
				"TestInterface",
				Comments.empty(),
				false,
				IArray.Empty,
	...
	describe("FlattenTrees - cross-type merging", () => {
		test("merge namespace and function with same name", () => {
			const ns = createMockNamespace(
				...
				Comments.empty(),
				false,
				IArray.fromArray<TsContainerOrDecl>([createMockClass("InnerClass")]),
			);
			const func = createMockFunction(
				"TestName",
				Comments.empty(),
				false,
				TsFunSig.simple(IArray.Empty, none),
			);
		...
		test("merge enum and class with same name", () => {
			...
			const classDecl = createMockClass(
				"TestName",
				Comments.empty(),
				false,
				false,
				...
				undefined,
				IArray.Empty,
				IArray.fromArray<TsMember>([createMockProperty("prop")]),
			);
			...
			const hasEnum = result.exists(
				(m) =>
					TsDeclEnum.isEnum(m) && (m as TsDeclEnum).name.value === "TestName",
			);
			const hasClass = result.exists(
				(m) =>
					TsDeclClass.isClass(m) &&
					(m as TsDeclClass).name.value === "TestName",
			);
		...
		test("merge type alias and interface with same name", () => {
			...
			expect(result.length).toBe(2); // Both should be preserved as they're different types
			const hasTypeAlias = result.exists(
				(m) =>
					TsDeclTypeAlias.isTypeAlias(m) &&
					(m as TsDeclTypeAlias).name.value === "TestName",
			);
			const hasInterface = result.exists(
				(m) =>
					TsDeclInterface.isInterface(m) &&
					(m as TsDeclInterface).name.value === "TestName",
			);
		...
		test("merge variable and class with same name", () => {
			...
			expect(result.length).toBe(2); // Both should be preserved as they're different types
			const hasVariable = result.exists(
				(m) => TsDeclVar.isVar(m) && (m as TsDeclVar).name.value === "TestName",
			);
			const hasClass = result.exists(
				(m) =>
					TsDeclClass.isClass(m) &&
		...
		test("merge augmented module and namespace", () => {
			const augModule = createMockAugmentedModule(
				"TestName",
				Comments.empty(),
				IArray.fromArray<TsContainerOrDecl>([createMockClass("ModuleClass")]),
			);
			const ns = createMockNamespace(
				"TestName",
				Comments.empty(),
				...
				IArray.fromArray<TsContainerOrDecl>([
					createMockClass("NamespaceClass"),
				]),
			);
			...
			expect(result.length).toBe(2); // Both should be preserved as they're different types
			const hasAugModule = result.exists(
				(m) =>
					TsAugmentedModule.isAugmentedModule(m) &&
					(m as TsAugmentedModule).name.value === "TestName",
			);
			const hasNamespace = result.exists(
				(m) =>
					TsDeclNamespace.isNamespace(m) &&
					(m as TsDeclNamespace).name.value === "TestName",
			);
		...
		test("merge different types with same name - priority order", () => {
			...
			const mergedNamespace = result.find((m) =>
				TsDeclNamespace.isNamespace(m),
			) as TsDeclNamespace;
		...
		test("merge function overloads", () => {
			const func1 = createMockFunction(
				"testFunc",
				Comments.empty(),
				false,
				TsFunSig.simple(
					IArray.fromArray([
						TsFunParam.typed(createSimpleIdent("x"), TsTypeRef.number),
					]),
					some(TsTypeRef.string),
				),
			);
			const func2 = createMockFunction(
				"testFunc",
				Comments.empty(),
				false,
				TsFunSig.simple(
					IArray.fromArray([
						TsFunParam.typed(createSimpleIdent("x"), TsTypeRef.string),
					]),
		...
		test("merge enum members", () => {
			...
			const enum1 = createMockEnum(
				"TestEnum",
				Comments.empty(),
				false,
				false,
				IArray.fromArray([
					TsEnumMember.create(Comments.empty(), createSimpleIdent("A"), none),
				]),
				true,
				JsLocation.zero(),
				createMockCodePath("test1"),
			);
			const enum2 = createMockEnum(
				"TestEnum",
				Comments.empty(),
				false,
				false,
				IArray.fromArray([
					TsEnumMember.create(Comments.empty(), createSimpleIdent("B"), none),
				]),
				true,
				JsLocation.zero(),
				createMockCodePath("test2"),
			);
