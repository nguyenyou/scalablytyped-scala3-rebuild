  ...
  describe('Edge Cases and Boundary Conditions', () => {
    ...
    it('intersection with function types', () => {
      ...
      if (result._tag === 'TsTypeIntersect') {
        ...
        expect(intersectResult.types.length).toBe(2);
        const resultTypes = intersectResult.types.toArray();
        expect(resultTypes.some((t: any) => t._tag === 'TsTypeFunction')).toBe(true);
        expect(resultTypes.some((t: any) => t._tag === 'TsTypeRef' && t.name.parts.apply(0).value === 'string')).toBe(true);
      }
    });
  ...
  describe('Type System Integration', () => {
    ...
    it('intersection with tuple types', () => {
      ...
      if (result._tag === 'TsTypeIntersect') {
        ...
        expect(intersectResult.types.length).toBe(2);
        const resultTypes = intersectResult.types.toArray();
        expect(resultTypes.some((t: any) => t._tag === 'TsTypeTuple')).toBe(true);
        expect(resultTypes.some((t: any) => t._tag === 'TsTypeRef' && t.name.parts.apply(0).value === 'boolean')).toBe(true);
      }
    });
  });
