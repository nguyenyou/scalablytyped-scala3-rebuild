...
describe("JsLocation", () => {
	...
	describe("JsLocation.Module - Construction and Basic Properties", () => {
		...
		test("Module construction with scoped module", () => {
			...
			const spec = ModuleSpec.specified(
				IArray.fromArray<TsIdent>([TsIdent.simple("fs")]),
			);
			const jsModule = JsLocation.module(module, spec);
	...
	describe("JsLocation.Both - Construction and Basic Properties", () => {
		...
		test("Both construction with complex paths", () => {
			const module = JsLocation.module(
				...
				ModuleSpec.specified(
					IArray.fromArray<TsIdent>([TsIdent.simple("Component")]),
				),
			);
			const global = JsLocation.global(
				TsQIdent.ofStrings("React", "Component"),
			);
			const both = JsLocation.both(module, global);
	...
	describe("ModuleSpec + operator (add identifier)", () => {
		...
		test("Any spec + namespaced identifier returns same spec", () => {
			...
			const specified = ModuleSpec.specified(
				IArray.fromArray<TsIdent>([TsIdent.simple("test")]),
			);

			expect(ModuleSpec.add(defaulted, TsIdent.namespaced())).toEqual(
				defaulted,
			);
			expect(ModuleSpec.add(namespaced, TsIdent.namespaced())).toEqual(
				namespaced,
			);
			expect(ModuleSpec.add(specified, TsIdent.namespaced())).toEqual(
				specified,
			);
		});
	});
	...
	describe("JsLocation / operator (tree navigation)", () => {
		test("Zero / TsDeclModule creates Module", () => {
			...
			const module = TsDeclModule.create(
				...
				CodePath.hasPath(
					TsIdent.simple("lib"),
					TsQIdent.of(TsIdent.simple("lib")),
				),
				JsLocation.zero(),
			);
	...
	describe("Edge Cases and Boundary Conditions", () => {
		...
		test("Module with complex scoped name", () => {
			const module = TsIdent.module(some("babel"), [
				"plugin",
				"transform",
				"runtime",
			]);
		...
		test("Global with very long path", () => {
			const longPath = TsQIdent.ofStrings(
