...
const createQIdent = (name: string): TsQIdent => TsQIdent.of(createSimpleIdent(name));
...
describe('OptionalType Tests', () => {
  ...
  describe('OptionalType - unapply method (pattern matching)', () => {
    test('extracts type from simple optional type', () => {
      const stringType = TsTypeRef.string;
      const optionalString = TsTypeUnion.create(IArray.apply<TsType>(stringType, TsTypeRef.undefined));
    ...
    test('extracts type from optional type with null', () => {
      ...
      const optionalNumber = TsTypeUnion.create(IArray.apply<TsType>(numberType, TsTypeRef.null));
    ...
    test('extracts type from optional type with both null and undefined', () => {
      ...
      const optionalBoolean = TsTypeUnion.create(IArray.apply<TsType>(booleanType, TsTypeRef.undefined, TsTypeRef.null));
    ...
    test('returns None for union without undefined or null', () => {
      const unionWithoutOptional = TsTypeUnion.create(IArray.apply<TsType>(TsTypeRef.string, TsTypeRef.number));
    ...
    test('handles complex union types', () => {
      const complexUnion = TsTypeUnion.create(IArray.apply<TsType>(
        TsTypeRef.string,
        TsTypeRef.number,
        TsTypeRef.undefined
      ));
      ...
      if (result._tag === 'Some') {
        ...
        expect(unionContainsType(extractedUnion, TsTypeRef.undefined)).toBe(false);
      }
    });
  });
  ...
  describe('OptionalType - nested optional types', () => {
    test('handles nested unapply on complex optional types', () => {
      ...
      const optionalType = TsTypeUnion.create(IArray.apply<TsType>(baseType, TsTypeRef.undefined));
      const nestedOptionalType = TsTypeUnion.create(IArray.apply<TsType>(optionalType, TsTypeRef.null));
    ...
    test('handles deeply nested optional types', () => {
      ...
      const level1 = TsTypeUnion.create(IArray.apply<TsType>(baseType, TsTypeRef.undefined));
      const level2 = TsTypeUnion.create(IArray.apply<TsType>(level1, TsTypeRef.null));
      const level3 = TsTypeUnion.create(IArray.apply<TsType>(level2, TsTypeRef.undefined));
    ...
    test('handles mixed optional types with multiple non-optional types', () => {
      ...
      const mixedOptional = TsTypeUnion.create(IArray.apply<TsType>(type1, type2, type3, TsTypeRef.undefined, TsTypeRef.null));
  ...
  describe('OptionalType - edge cases', () => {
    test('handles union with only undefined', () => {
      const onlyUndefined = TsTypeUnion.create(IArray.apply<TsType>(TsTypeRef.undefined));
    ...
    test('handles union with only null and undefined', () => {
      const onlyOptionals = TsTypeUnion.create(IArray.apply<TsType>(TsTypeRef.undefined, TsTypeRef.null));
    ...
    test('handles single type union', () => {
      const singleTypeUnion = TsTypeUnion.create(IArray.apply<TsType>(TsTypeRef.string));
  ...
  describe('OptionalType - complex type structures', () => {
    ...
    test('handles intersection types', () => {
      const intersectionType = TsTypeIntersect.create(IArray.apply<TsType>(TsTypeRef.string, TsTypeRef.number));
    ...
    test('handles generic types', () => {
      const genericType = TsTypeRef.create(Comments.empty(), createQIdent('Array'), IArray.apply<TsType>(TsTypeRef.string));
  ...
  describe('OptionalType - pattern matching scenarios', () => {
    test('can be used in pattern matching', () => {
      const optionalString = TsTypeUnion.create(IArray.apply<TsType>(TsTypeRef.string, TsTypeRef.undefined));
    ...
    test('pattern matching with nested optional types', () => {
      const baseType = createMockTypeRef('BaseType');
      const level1Optional = TsTypeUnion.create(IArray.apply<TsType>(baseType, TsTypeRef.undefined));
      const level2Optional = TsTypeUnion.create(IArray.apply<TsType>(level1Optional, TsTypeRef.null));
  ...
  describe('OptionalType - performance and edge cases', () => {
    test('handles large union types efficiently', () => {
      const manyTypes = Array.from({ length: 50 }, (_, i) => createMockTypeRef(`Type${i + 1}`));
      const largeUnion = TsTypeUnion.create(IArray.fromArray<TsType>(manyTypes).append(TsTypeRef.undefined));
    ...
    test('handles union with duplicated optional types', () => {
      const stringType = TsTypeRef.string;
      const unionWithDuplicates = TsTypeUnion.create(IArray.apply<TsType>(
        stringType,
        TsTypeRef.undefined,
        TsTypeRef.null,
    ...
    test('handles complex nested union structures', () => {
      const innerUnion = TsTypeUnion.create(IArray.apply<TsType>(TsTypeRef.string, TsTypeRef.number));
      const outerUnion = TsTypeUnion.create(IArray.apply<TsType>(innerUnion, TsTypeRef.undefined));
  ...
  describe('OptionalType - convenience functions', () => {
    ...
    test('extractOptional is equivalent to OptionalType.unapply', () => {
      const optionalType = TsTypeUnion.create(IArray.apply<TsType>(TsTypeRef.string, TsTypeRef.undefined));
