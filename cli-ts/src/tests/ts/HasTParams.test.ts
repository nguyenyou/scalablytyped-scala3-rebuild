...
const createMockTypeFunction = (
	tparams: IArray<TsTypeParam> = IArray.Empty,
): TsTypeFunction => TsTypeFunction.create(createMockFunSig(tparams));
...
const createMockTypeConditional = (
	...
	return TsTypeConditional.create(pred, TsTypeRef.string, TsTypeRef.any);
};
...
describe("HasTParams Tests", () => {
	...
	describe("HasTParams - unapply method - Positive Cases", () => {
		...
		test("TsMemberFunction with signature type parameters", () => {
			const tparam = createMockTypeParam("T");
			const mockMemberFunction = createMockMemberFunction(
				"testMethod",
				IArray.apply(tparam),
			);
	...
	describe("HasTParams - Edge Cases", () => {
		test("multiple type parameters with constraints", () => {
			...
			const tparam3 = createMockTypeParam(
				"V",
				TsTypeRef.any,
				TsTypeRef.boolean,
			);
			...
			const mockClass = createMockClass(
				"TestClass",
				IArray.apply(tparam1, tparam2, tparam3),
			);
			const result = HasTParams.unapply(mockClass);
	...
	describe("HasTParams - Complex Scenarios", () => {
		...
		test("type parameter names and constraints preservation", () => {
			const tparam = createMockTypeParam(
				"CustomName",
				TsTypeRef.string,
				TsTypeRef.any,
			);
			...
			if (retrievedParam.upperBound._tag === "Some") {
				expect(retrievedParam.upperBound.value.asString).toBe(
					TsTypeRef.string.asString,
				);
			}
			if (retrievedParam.default._tag === "Some") {
				expect(retrievedParam.default.value.asString).toBe(
					TsTypeRef.any.asString,
				);
			}
		});
	});
	...
	describe("HasTParams - Error Handling and Robustness", () => {
		...
		test("handles mixed empty and non-empty scenarios", () => {
			...
			const nonEmptyClass = createMockClass(
				"NonEmptyClass",
				IArray.apply(createMockTypeParam("T")),
			);
