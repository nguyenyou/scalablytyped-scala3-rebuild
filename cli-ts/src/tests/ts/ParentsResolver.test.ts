...
const createTypeRef = (name: string, tparams: IArray<TsType> = IArray.Empty): TsTypeRef =>
  TsTypeRef.create(Comments.empty(), createQIdent(name), tparams);
...
const createMockScope = (declarations: Map<string, TsNamedDecl>): TsTreeScope => {
  const scope: TsTreeScope = {
    ...
    lookupIncludeScope: (qname: TsQIdent, skipValidation = false): IArray<[TsNamedDecl, TsTreeScope]> => {
      const name = qname.parts.get(0)?.value;
      const decl = name ? declarations.get(name) : undefined;
      return decl ? IArray.apply([decl, scope] as [TsNamedDecl, TsTreeScope]) : IArray.Empty;
    },
    ...
    '/'(current: any): any { return scope; },
    get root(): any { return scope; },
    get lookupUnqualified(): boolean { return false; },
    get logger(): any { return { fatalMaybe: () => {} }; },
    get stack(): any[] { return []; },
    get tparams(): Map<any, any> { return new Map(); },
    get tkeys(): Set<any> { return new Set(); },
    get parent(): TsTreeScope { return scope; },
    get moduleScopes(): Map<any, any> { return new Map(); },
    get moduleAuxScopes(): Map<any, any> { return new Map(); },
    get exports(): IArray<any> { return IArray.Empty; },
...
const createDeclarationsMap = (...entries: [string, TsNamedDecl][]): Map<string, TsNamedDecl> => {
...
describe('ParentsResolver Tests', () => {
  describe('ParentsResolver - basic functionality', () => {
    ...
    test('resolves interface with single parent interface', () => {
      ...
      const scope = createMockScope(createDeclarationsMap(
    ...
    test('resolves class with implemented interfaces', () => {
      ...
      const parentNames = result.parents.toArray().map(p => (p as TsDeclInterface | TsDeclClass).name.value);
  ...
  describe('ParentsResolver - unresolved types', () => {
    ...
    test('handles mix of resolved and unresolved types', () => {
      ...
      expect((result.parents.get(0) as TsDeclInterface).name.value).toBe('KnownInterface');
  ...
  describe('ParentsResolver - type aliases', () => {
    test('resolves type alias to interface', () => {
      ...
      expect((result.parents.get(0) as TsDeclInterface).name.value).toBe('TargetInterface');
    });
    ...
    test('resolves type alias to union type', () => {
      const interface1 = createMockInterface('Interface1');
      const interface2 = createMockInterface('Interface2');
      const unionType = TsTypeUnion.create(IArray.apply(
    ...
    test('resolves type alias to intersection type', () => {
      const interface1 = createMockInterface('Interface1');
      const interface2 = createMockInterface('Interface2');
      const intersectionType = TsTypeIntersect.create(IArray.apply<TsType>(
        createTypeRef('Interface1'),
        createTypeRef('Interface2')
      ));
      const typeAlias = createMockTypeAlias('AliasToIntersection', intersectionType);
      const interface_ = createMockInterface(
        'TestInterface',
        IArray.Empty,
  ...
  describe('ParentsResolver - circular references', () => {
    test('handles circular inheritance (interface)', () => {
      ...
      expect(result.parents.length).toBe(1);
      expect((result.parents.get(0) as TsDeclInterface).name.value).toBe('Interface2');
    });
  ...
  describe('ParentsResolver - edge cases', () => {
    ...
    test('handles non-interface/class declarations in scope', () => {
      ...
      const primitiveAlias = createMockTypeAlias('StringAlias', createTypeRef('string'));
      const interface_ = createMockInterface(
        'TestInterface',
        IArray.Empty,
    ...
    test('handles duplicate parent references', () => {
      ...
      expect(result.parents.length).toBe(1);
      expect(result.unresolved.length).toBe(0);
      expect((result.parents.get(0) as TsDeclInterface).name.value).toBe('ParentInterface');
    });
  });
  ...
  describe('ParentsResolver - utility functions', () => {
    test('WithParents.create works correctly', () => {
      const interface_ = createMockInterface('TestInterface');
      const parents = IArray.apply<InterfaceOrClass>(createMockInterface('Parent') as InterfaceOrClass);
      const unresolved = IArray.apply<TsType>(createTypeRef('Unknown') as TsType);
