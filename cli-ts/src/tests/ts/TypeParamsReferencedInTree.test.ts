...
describe("TypeParamsReferencedInTree Tests", () => {
	...
	describe("Basic Functionality", () => {
		...
		test("single type parameter reference", () => {
			...
			const classWithMember = createMockClass(
				"TestClass",
				IArray.Empty,
				IArray.apply<TsMember>(member),
			);
		...
		test("multiple type parameter references", () => {
			...
			const classWithMembers = createMockClass(
				"TestClass",
				IArray.Empty,
				IArray.apply<TsMember>(memberT, memberU),
			);
	...
	describe("Scope Shadowing", () => {
		test("inner scope shadows outer type parameter", () => {
			...
			const scope = new Map<TsIdent, TsTypeParam>([
				[outerTparam.name, outerTparam],
			]);

			const typeRef = createTypeRef("T");
			const member = createMemberProperty("prop", typeRef);
			const innerClass = createMockClass(
				"InnerClass",
				IArray.apply(innerTparam),
				IArray.apply<TsMember>(member),
			);
	...
	describe("Different Tree Types", () => {
		test("type parameter reference in interface", () => {
			...
			const typeRef = createTypeRef("T");
			const member = createMemberProperty("prop", typeRef);
			const interfaceMock = createMockInterface(
				"TestInterface",
				IArray.Empty,
				IArray.apply<TsMember>(member),
			);
	...
	describe("Edge Cases and Error Conditions", () => {
		...
		test("type parameter with generic type arguments", () => {
			...
			const arrayTypeRef = createTypeRef(
				"Array",
		...
		test("duplicate type parameter references", () => {
			...
			const classWithMembers = createMockClass(
				"TestClass",
				IArray.Empty,
				IArray.apply<TsMember>(member1, member2),
			);
		...
		test("type parameter with complex bound structure", () => {
			...
			const complexBound = createTypeRefAsType(
				"Array",
				IArray.apply(createTypeRefAsType("T")),
			);
	...
	describe("Performance and Stress Tests", () => {
		test("large number of type parameters", () => {
			const tparams = Array.from({ length: 50 }, (_, i) =>
				createTypeParam(`T${i + 1}`),
			);
			const scope = new Map<TsIdent, TsTypeParam>(
				tparams.map((tp) => [tp.name, tp]),
			);
			...
			const members = tparams
				...
				.filter(({ i }) => i % 2 === 0)
				.map(({ tp, i }) =>
					createMemberProperty(`prop${i}`, createTypeRef(tp.name.value)),
				);

			const classWithMembers = createMockClass(
				"LargeClass",
				IArray.Empty,
				IArray.fromArray<TsMember>(members),
			);
			...
			expect(
				result.forall((tp: TsTypeParam) => tp.name.value.startsWith("T")),
			).toBe(true);
		});
		...
		test("deeply nested type references", () => {
			...
			const deepTypeRef = createTypeRef(
				"Promise",
				IArray.apply(
					createTypeRefAsType(
						"Array",
		...
		test("complex tree with many different constructs", () => {
			...
			const prop3 = createMemberProperty(
				"prop3",
				createTypeRef("Array", IArray.apply(createTypeRefAsType("T"))),
			);
			...
			const funSig = TsFunSig.create(
				...
				IArray.apply(
					TsFunParam.create(
						Comments.empty(),
						createSimpleIdent("param"),
						some(createTypeRef("U")),
					),
				),
			...
			const classWithMembers = createMockClass(
				"ComplexClass",
				IArray.Empty,
				IArray.apply<TsMember>(prop1, prop2, prop3, method),
			);
