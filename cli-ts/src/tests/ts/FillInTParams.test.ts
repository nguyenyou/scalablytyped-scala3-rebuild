...
describe('FillInTParams', () => {
  describe('Interface Type Parameter Substitution', () => {
    ...
    it('single type parameter substitution', () => {
      ...
      const interface_ = createMockInterface('TestInterface', IArray.apply(tparam));
      const providedType = createTypeRefAsType('string');

      const result = FillInTParams.apply(interface_, IArray.apply(providedType));
    ...
    it('fewer provided than expected uses defaults or warnings', () => {
      const tparamWithDefault = createTypeParam('T', undefined, createTypeRef('string'));
      const tparamWithoutDefault = createTypeParam('U');
      const interface_ = createMockInterface('TestInterface', IArray.fromArray([tparamWithDefault, tparamWithoutDefault]));
  ...
  describe('Type Alias Substitution', () => {
    ...
    it('single type parameter substitution', () => {
      const tparam = createTypeParam('T');
      const aliasType = createTypeRef('Array', IArray.apply(createTypeRefAsType('T')));
      const alias = createMockTypeAlias('TestAlias', IArray.apply(tparam), aliasType);
      const providedType = createTypeRefAsType('string');
  ...
  describe('Edge Cases and Error Conditions', () => {
    ...
    it('large parameter lists', () => {
      const manyTParams = Array.from({ length: 50 }, (_, i) => createTypeParam(`T${i + 1}`));
      const manyProvidedTypes = Array.from({ length: 50 }, (_, i) => createTypeRefAsType(`Type${i + 1}`));

      const interface_ = createMockInterface('TestInterface', IArray.fromArray(manyTParams));
      const result = FillInTParams.apply(interface_, IArray.fromArray(manyProvidedTypes));
    ...
    it('boundary conditions with parameter count mismatches', () => {
      ...
      const resultPartial = FillInTParams.apply(interface_, IArray.apply(createTypeRefAsType('string')));
      expect(resultPartial.tparams.length).toBe(0);

      // Test with many provided types (more than expected)
      const manyTypes = Array.from({ length: 10 }, (_, i) => createTypeRefAsType(`Type${i + 1}`));
      const resultMany = FillInTParams.apply(interface_, IArray.fromArray(manyTypes));
  ...
  describe('Integration with TypeRewriter', () => {
    ...
    it('handles complex nested type substitution', () => {
      const tparam = createTypeParam('T');
      const nestedType = createTypeRef('Array', IArray.apply(createTypeRefAsType('T')));
      const alias = createMockTypeAlias('TestAlias', IArray.apply(tparam), nestedType);
      const providedType = createTypeRefAsType('string');
