...
function createMockProperty(
	name: string,
	tpe: TsTypeRef = TsTypeRef.string,
): TsMemberProperty {
...
describe("AddComments", () => {
	...
	describe("TsMemberCall Processing", () => {
		...
		test("concatenates comments to TsMemberCall with existing comments", () => {
			...
			expect(
				resultCall.comments.cs.some(
					(c) => c instanceof Raw && c.raw === "existing",
				),
			).toBe(true);
			expect(
				resultCall.comments.cs.some((c) => c instanceof Raw && c.raw === "new"),
			).toBe(true);
		});
	...
	describe("TsMemberProperty Processing", () => {
		...
		test("concatenates comments to TsMemberProperty with existing comments", () => {
			...
			expect(
				resultProperty.comments.cs.some(
					(c) => c instanceof Raw && c.raw === "existing property",
				),
			).toBe(true);
			expect(
				resultProperty.comments.cs.some(
					(c) => c instanceof Raw && c.raw === "new property",
				),
			).toBe(true);
		});
	});
	...
	describe("Edge Cases and Error Handling", () => {
		test("handles multiple comments in newComments", () => {
			...
			expect(
				resultProperty.comments.cs.some(
					(c) => c instanceof Raw && c.raw === "comment 1",
				),
			).toBe(true);
			expect(
				resultProperty.comments.cs.some(
					(c) => c instanceof Raw && c.raw === "comment 2",
				),
			).toBe(true);
			expect(
				resultProperty.comments.cs.some(
					(c) => c instanceof Raw && c.raw === "comment 3",
				),
			).toBe(true);
		});
		...
		test("handles complex comment types", () => {
			...
			expect(
				resultCall.comments.cs.some(
					(c) => c instanceof Raw && c.raw === "raw comment",
				),
			).toBe(true);
			expect(
				resultCall.comments.cs.some(
					(c) => c instanceof NameHint && c.value === "hint",
				),
			).toBe(true);
		});
	});
