...
describe("ExpandTypeMappings", () => {
	describe("Basic Functionality", () => {
		...
		test("After object exists and extends TreeTransformationScopedChanges", () => {
			...
			expect(typeof ExpandTypeMappingsAfterTransform.enterTsType).toBe(
				"function",
			);
		});
	...
	describe("Type Alias Processing", () => {
		...
		test("enterTsDecl with type alias - union type", () => {
			...
			const unionType = TsTypeUnion.simplified(
				IArray.apply<TsType>(stringType, numberType),
			);
	...
	describe("AllMembersFor - Basic Functionality", () => {
		...
		test("forType with TsTypeIntersect - single type", () => {
			...
			const intersection = TsTypeIntersect.create(
				IArray.apply(typeRef as TsType),
			);
	...
	describe("evaluateKeys - Basic Functionality", () => {
		...
		test("evaluateKeys with TsTypeObject - with properties", () => {
			...
			const objectType = TsTypeObject.create(
				Comments.empty(),
				IArray.apply(prop1 as TsMember, prop2 as TsMember),
			);
			...
			if (result._tag === "Ok") {
				...
				const keyNames = Array.from(result.value).map(
					(k) => (k.lit as any).value,
				);
		...
		test("evaluateKeys with TsTypeUnion - with literals", () => {
			...
			const unionType = TsTypeUnion.create(
				IArray.apply(literal1 as TsType, literal2 as TsType),
			);
	...
	describe("After - Functionality", () => {
		test("After.enterTsType with interface", () => {
			...
			const result =
				ExpandTypeMappingsAfterTransform.enterTsType(scope)(interface_);
		...
		test("After.enterTsType with type alias", () => {
			...
			const result =
				ExpandTypeMappingsAfterTransform.enterTsType(scope)(typeAlias);
	...
	describe("Edge Cases and Error Handling", () => {
		test("handles null/empty inputs gracefully", () => {
			...
			const result =
				ExpandTypeMappingsTransform.enterTsDecl(scope)(emptyInterface);
			expect(result).toBe(emptyInterface);
		});

		test("handles complex nested types", () => {
			const scope = createMockScope();
			const nestedType = TsTypeIntersect.create(
				IArray.apply(
