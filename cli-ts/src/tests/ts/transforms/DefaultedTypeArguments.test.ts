...
import { TsIdentLibrarySimple } from '@/internal/ts/trees.js';
...
describe('DefaultedTypeArguments', () => {
  describe('Basic Functionality', () => {
    ...
    test('handles type reference with no missing arguments', () => {
      ...
      const interface_ = createMockInterface("TestInterface", IArray.fromArray([typeParam1, typeParam2]));
      const scope = createMockScope(interface_);
      
      const typeRef = createTypeRef("TestInterface", IArray.fromArray([TsTypeRef.string as TsType, TsTypeRef.number as TsType]));
      const transform = new DefaultedTypeArguments();
  ...
  describe('Default Type Parameter Handling', () => {
    test('adds default type arguments when missing', () => {
      ...
      const typeRef = createTypeRef("TestInterface", IArray.fromArray([TsTypeRef.boolean as TsType]));
      const transform = new DefaultedTypeArguments();
      const result = transform.enterTsTypeRef(scope)(typeRef);
  ...
  describe('Self-Reference Handling', () => {
    test('handles self-referencing default types', () => {
      const selfRefDefault = createTypeRef("TestInterface", IArray.fromArray<TsType>([TsTypeRef.string]));
      const typeParam1 = createTypeParam("T", some(selfRefDefault));
      const interface_ = createMockInterface("TestInterface", IArray.fromArray([typeParam1]));
      const scope = createMockScope(interface_);
  ...
  describe('Edge Cases', () => {
    test('handles non-existent type references', () => {
      ...
      const typeRef = createTypeRef("NonExistentType", IArray.fromArray([TsTypeRef.string as TsType]));
      const transform = new DefaultedTypeArguments();
      const result = transform.enterTsTypeRef(scope)(typeRef);
    ...
    test('handles type aliases with defaults', () => {
      const typeParam1 = createTypeParam("T", some(TsTypeRef.string));
      const typeParam2 = createTypeParam("U", some(TsTypeRef.number));
      const typeAlias = createMockTypeAlias("TestAlias", IArray.fromArray([typeParam1, typeParam2]));
    ...
    test('handles mixed provided and default arguments', () => {
      ...
      const interface_ = createMockInterface("TestInterface", IArray.fromArray([typeParam1, typeParam2, typeParam3]));
      const scope = createMockScope(interface_);
      
      const typeRef = createTypeRef("TestInterface", IArray.fromArray<TsType>([TsTypeRef.any, TsTypeRef.void]));
      const transform = new DefaultedTypeArguments();
      const result = transform.enterTsTypeRef(scope)(typeRef);
    ...
    test('preserves comments and other properties', () => {
      ...
      const typeRef = TsTypeRef.create(originalComments, createQIdent("TestInterface"), IArray.Empty);
      const transform = new DefaultedTypeArguments();
      const result = transform.enterTsTypeRef(scope)(typeRef);
  ...
  describe('Integration Scenarios', () => {
    test('works with complex nested type structures', () => {
      const typeParam1 = createTypeParam("T", some(TsTypeRef.string));
      const typeParam2 = createTypeParam("U", some(TsTypeRef.number));
      const interface_ = createMockInterface("Container", IArray.fromArray([typeParam1, typeParam2]));
      const scope = createMockScope(interface_);
      
      const nestedTypeRef = createTypeRef("Container", IArray.fromArray<TsType>([TsTypeRef.boolean]));
      const transform = new DefaultedTypeArguments();
    ...
    test('handles multiple type declarations in scope', () => {
      const typeParam1 = createTypeParam("T", some(TsTypeRef.string));
      const typeParam2 = createTypeParam("U", some(TsTypeRef.number));
      const interface1 = createMockInterface("Interface1", IArray.fromArray([typeParam1]));
      const interface2 = createMockInterface("Interface2", IArray.fromArray([typeParam1, typeParam2]));
