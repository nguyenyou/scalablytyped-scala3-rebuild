...
function createMockMemberFunction(name: string, comments: Comments = Comments.empty()): TsMemberFunction {
...
function createMockMemberCall(comments: Comments = Comments.empty()): TsMemberCall {
...
function createMockMemberCtor(comments: Comments = Comments.empty()): TsMemberCtor {
...
function createMockDeclFunction(name: string, comments: Comments = Comments.empty()): TsDeclFunction {
...
describe('RemoveComment', () => {
  describe('RemoveComment trait - Basic Functionality', () => {
    it('remove method exists for all supported types', () => {
      ...
      const memberFunction = createMockMemberFunction('test', createCommentsWithRaw('test comment'));
      const memberCall = createMockMemberCall(createCommentsWithRaw('call comment'));
      const memberCtor = createMockMemberCtor(createCommentsWithRaw('ctor comment'));
      const declFunction = createMockDeclFunction('testFunc', createCommentsWithRaw('func comment'));
  ...
  describe('TsMemberFunction - Comment Removal', () => {
    it('removes single comment', () => {
      ...
      const memberFunction = createMockMemberFunction('testMethod', originalComments);
    ...
    it('removes multiple comments', () => {
      const originalComments = createCommentsWithMultiple('Comment 1', 'Comment 2', 'Comment 3');
      const memberFunction = createMockMemberFunction('testMethod', originalComments);
    ...
    it('handles already empty comments', () => {
      const memberFunction = createMockMemberFunction('testMethod', Comments.empty());
  ...
  describe('TsMemberCall - Comment Removal', () => {
    ...
    it('removes multiple comments', () => {
      const originalComments = createCommentsWithMultiple('Call comment 1', 'Call comment 2');
      const memberCall = createMockMemberCall(originalComments);
  ...
  describe('TsMemberCtor - Comment Removal', () => {
    it('removes single comment', () => {
      const originalComments = createCommentsWithRaw('Constructor signature comment');
    ...
    it('removes multiple comments', () => {
      const originalComments = createCommentsWithMultiple('Ctor comment 1', 'Ctor comment 2', 'Ctor comment 3');
      const memberCtor = createMockMemberCtor(originalComments);
  ...
  describe('TsDeclFunction - Comment Removal', () => {
    it('removes single comment', () => {
      const originalComments = createCommentsWithRaw('Function declaration comment');
    ...
    it('removes multiple comments', () => {
      const originalComments = createCommentsWithMultiple('Func comment 1', 'Func comment 2');
      const declFunction = createMockDeclFunction('testFunc', originalComments);
  ...
  describe('keepFirstOnly method - Basic Functionality', () => {
    ...
    it('single element array preserves comments', () => {
      const singleFunction = createMockMemberFunction('single', createCommentsWithRaw('Keep this comment'));
    ...
    it('multiple elements - first keeps comments, rest lose comments', () => {
      const func1 = createMockMemberFunction('first', createCommentsWithRaw('Keep this comment'));
      const func2 = createMockMemberFunction('second', createCommentsWithRaw('Remove this comment'));
      const func3 = createMockMemberFunction('third', createCommentsWithRaw('Remove this too'));
    ...
    it('works with TsMemberCall', () => {
      const call1 = createMockMemberCall(createCommentsWithRaw('Keep this call comment'));
      const call2 = createMockMemberCall(createCommentsWithRaw('Remove this call comment'));
    ...
    it('works with TsMemberCtor', () => {
      const ctor1 = createMockMemberCtor(createCommentsWithRaw('Keep this ctor comment'));
      const ctor2 = createMockMemberCtor(createCommentsWithRaw('Remove this ctor comment'));
      const ctor3 = createMockMemberCtor(createCommentsWithRaw('Remove this ctor comment too'));
    ...
    it('works with TsDeclFunction', () => {
      const decl1 = createMockDeclFunction('first', createCommentsWithRaw('Keep this decl comment'));
      const decl2 = createMockDeclFunction('second', createCommentsWithRaw('Remove this decl comment'));
  ...
  describe('keepFirstOnly method - Edge Cases and Boundary Conditions', () => {
    it('first element has no comments, others have comments', () => {
      ...
      const func2 = createMockMemberFunction('second', createCommentsWithRaw('This will be removed'));
      const func3 = createMockMemberFunction('third', createCommentsWithRaw('This will also be removed'));
      const array = IArray.fromArray([func1, func2, func3]);
    ...
    it('preserves order of elements', () => {
      const func1 = createMockMemberFunction('alpha', createCommentsWithRaw('Keep'));
      const func2 = createMockMemberFunction('beta', createCommentsWithRaw('Remove'));
      const func3 = createMockMemberFunction('gamma', createCommentsWithRaw('Remove'));
      const func4 = createMockMemberFunction('delta', createCommentsWithRaw('Remove'));
    ...
    it('handles large arrays efficiently', () => {
      const functions = Array.from({ length: 100 }, (_, i) =>
        createMockMemberFunction(`func${i + 1}`, createCommentsWithRaw(`Comment ${i + 1}`))
      );
    ...
    it('mixed types with first element having multiple comments', () => {
      const multipleComments = createCommentsWithMultiple('Comment 1', 'Comment 2', 'Comment 3');
      const func1 = createMockMemberFunction('first', multipleComments);
      const func2 = createMockMemberFunction('second', createCommentsWithRaw('Single comment'));
      const array = IArray.fromArray([func1, func2]);
  ...
  describe('Integration Tests - Real-world Scenarios', () => {
    it('processing overloaded functions', () => {
      ...
      const overload1 = createMockDeclFunction('process', createCommentsWithRaw('Main documentation for process function'));
      const overload2 = createMockDeclFunction('process', createCommentsWithRaw('Overload 1 documentation'));
      const overload3 = createMockDeclFunction('process', createCommentsWithRaw('Overload 2 documentation'));
      ...
      expect(result.length).toBe(3);
      expect(result.get(0).comments.nonEmpty).toBe(true);
      expect(result.get(0).comments.rawCs[0]).toBe('Main documentation for process function');
      expect(result.get(1).comments.isEmpty).toBe(true);
      expect(result.get(2).comments.isEmpty).toBe(true);

      // All should have the same name
      expect(result.toArray().every(f => f.name.value === 'process')).toBe(true);
    });

    it('processing constructor overloads', () => {
      const ctor1 = createMockMemberCtor(createCommentsWithRaw('Primary constructor documentation'));
      const ctor2 = createMockMemberCtor(createCommentsWithRaw('Alternative constructor'));
      ...
      expect(result.length).toBe(2);
      expect(result.get(0).comments.nonEmpty).toBe(true);
      expect(result.get(0).comments.rawCs[0]).toBe('Primary constructor documentation');
      expect(result.get(1).comments.isEmpty).toBe(true);
    });

    it('processing call signatures', () => {
      const call1 = createMockMemberCall(createCommentsWithRaw('Primary call signature'));
      const call2 = createMockMemberCall(createCommentsWithRaw('Alternative call signature'));
      const call3 = createMockMemberCall(createCommentsWithRaw('Third call signature'));
    ...
    it('type consistency - keepFirstOnly works with homogeneous arrays', () => {
      ...
      const func1 = createMockMemberFunction('method1', createCommentsWithRaw('Method comment 1'));
      const func2 = createMockMemberFunction('method2', createCommentsWithRaw('Method comment 2'));
      const func3 = createMockMemberFunction('method3', createCommentsWithRaw('Method comment 3'));
      const functions = IArray.fromArray([func1, func2, func3]);
      const resultFunctions = RemoveComment.keepFirstOnly(functions, RemoveComment.r1);
      ...
      const call1 = createMockMemberCall(createCommentsWithRaw('Call comment 1'));
      const call2 = createMockMemberCall(createCommentsWithRaw('Call comment 2'));
      ...
      const ctor1 = createMockMemberCtor(createCommentsWithRaw('Ctor comment 1'));
      const ctor2 = createMockMemberCtor(createCommentsWithRaw('Ctor comment 2'));
      const ctors = IArray.fromArray([ctor1, ctor2]);
  ...
  describe('Comment Content Preservation', () => {
    it('preserves exact comment content in first element', () => {
      const originalComment = 'This is a very specific comment with special characters: @param {string} name - The name parameter';
      const func1 = createMockMemberFunction('test', createCommentsWithRaw(originalComment));
      const func2 = createMockMemberFunction('test2', createCommentsWithRaw('This will be removed'));
      const array = IArray.fromArray([func1, func2]);
    ...
    it('preserves multiple comments in first element', () => {
      ...
      const multiComments = createCommentsWithMultiple(comment1, comment2, comment3);
      const func1 = createMockMemberFunction('test', multiComments);
      const func2 = createMockMemberFunction('test2', createCommentsWithRaw('Remove this'));
      const array = IArray.fromArray([func1, func2]);
