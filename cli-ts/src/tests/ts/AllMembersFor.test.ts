...
describe('AllMembersFor', () => {
  describe('forType', () => {
    it('returns empty for TsTypeUnion', () => {
      ...
      const unionType = TsTypeUnion.create(IArray.fromArray([TsTypeRef.string, TsTypeRef.number] as TsType[]));
    ...
    it('returns members for TsTypeObject', () => {
      ...
      const objectType = TsTypeObject.create(Comments.empty(), IArray.fromArray<TsMember>([property]));
    ...
    it('returns empty for primitive types', () => {
      ...
      const primitiveTypes = [
        ...
        TsTypeFunction.create(TsFunSig.create(Comments.empty(), IArray.Empty, IArray.Empty, some(TsTypeRef.void))),
        TsTypeConstructorConstructor.create(false, TsTypeFunction.create(TsFunSig.create(Comments.empty(), IArray.Empty, IArray.Empty, some(TsTypeRef.void)))),
        ...
        TsTypeLookup.create(TsTypeRef.string, TsTypeLiteral.create(TsLiteral.str('key'))),
        TsTypeThis.create()
      ];
    ...
    it('handles TsTypeIntersect by flattening members', () => {
      ...
      const intersectType = TsTypeIntersect.create(IArray.fromArray<TsType>([obj1, obj2]));
  ...
  describe('handleOverridingFields', () => {
    it('combines members without conflicts', () => {
      ...
      const result = AllMembersFor.handleOverridingFields(fromThis, fromParents);
    ...
    it('filters out overridden properties from parents', () => {
      ...
      const fromParents = IArray.fromArray<TsMember>([parentProp, otherProp, method]);
      ...
      const parentPropsInResult = result.toArray().filter(m =>
        m._tag === 'TsMemberProperty' && (m as TsMemberProperty).name.value === 'sameName'
      );
    ...
    it('handles empty collections', () => {
      const result1 = AllMembersFor.handleOverridingFields(IArray.Empty, IArray.Empty);
      expect(result1.isEmpty).toBe(true);

      const prop = createMockProperty('prop');
      const result2 = AllMembersFor.handleOverridingFields(IArray.fromArray<TsMember>([prop]), IArray.Empty);
      expect(result2.length).toBe(1);
      expect(result2.apply(0)).toBe(prop);

      const result3 = AllMembersFor.handleOverridingFields(IArray.Empty, IArray.fromArray<TsMember>([prop]));
  ...
  describe('apply', () => {
    it('returns empty for circular reference', () => {
      ...
      const loopDetector = loopResult._tag === 'Right' ? loopResult.right : LoopDetector.initial;
  ...
  describe('forInterface', () => {
    it('handles interface without inheritance', () => {
      ...
      const interfaceDecl = createMockInterface('TestInterface', IArray.fromArray<TsMember>([prop]));
      const scope = createMockScope();
      const loopDetector = createLoopDetector();

      const result = AllMembersFor.forInterface(loopDetector, interfaceDecl, scope, IArray.Empty);
    ...
    it('handles interface with inheritance', () => {
      const prop1 = createMockProperty('prop1');
      const interfaceDecl = createMockInterface('TestInterface', IArray.fromArray<TsMember>([prop1]), IArray.fromArray<TsTypeRef>([createTypeRef('BaseInterface')]));
      const scope = createMockScope();
      const loopDetector = createLoopDetector();
    ...
    it('handles interface with type parameters', () => {
      ...
      const interfaceDecl = createMockInterface('GenericInterface', IArray.fromArray<TsMember>([prop]));
      const scope = createMockScope();
      const loopDetector = createLoopDetector();
      const tparams = IArray.fromArray<TsType>([TsTypeRef.string]);

      const result = AllMembersFor.forInterface(loopDetector, interfaceDecl, scope, tparams);
  ...
  describe('Complex Scenarios', () => {
    it('handles multiple property overrides correctly', () => {
      ...
      const fromParents = IArray.fromArray<TsMember>([parentProp1, parentProp2, parentMethod]);
      ...
      const namePropsInResult = result.toArray().filter(m =>
        m._tag === 'TsMemberProperty' && (m as TsMemberProperty).name.value === 'name'
      );
  ...
  describe('Error Handling', () => {
    it('handles null or invalid inputs gracefully', () => {
      ...
      const result2 = AllMembersFor.forType(scope, loopDetector)(emptyIntersect);
    ...
    it('handles deeply nested type intersections', () => {
      ...
      const obj1 = TsTypeObject.create(Comments.empty(), IArray.fromArray<TsMember>([prop1]));
      const obj2 = TsTypeObject.create(Comments.empty(), IArray.fromArray<TsMember>([prop2]));
      const obj3 = TsTypeObject.create(Comments.empty(), IArray.fromArray<TsMember>([prop3]));

      const nestedIntersect = TsTypeIntersect.create(IArray.fromArray<TsType>([
        obj1,
        TsTypeIntersect.create(IArray.fromArray<TsType>([obj2, obj3]))
      ]));

      const result = AllMembersFor.forType(scope, loopDetector)(nestedIntersect);
