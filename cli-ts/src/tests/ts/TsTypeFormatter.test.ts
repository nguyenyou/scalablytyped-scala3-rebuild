...
function createTypeRef(name: string, ...tparams: TsType[]): TsTypeRef {
  return TsTypeRef.create(Comments.empty(), createQIdent(name), IArray.fromArray(tparams));
}
  ...
  describe('Basic Functionality', () => {
    describe('qident', () => {
      test('formats qualified identifiers correctly', () => {
        ...
        expect(formatter.qident(qident3)).toBe('MyNamespace.SubNamespace.MyClass');
    ...
    describe('tparams', () => {
      test('formats type parameters correctly', () => {
        ...
        const emptyResult = formatter.tparams(IArray.Empty, (x: string) => x.toString());
        expect(O.isNone(emptyResult)).toBe(true);

        // Single type parameter
        const tparams1 = IArray.fromArray([createTypeParam('T')]);
        const result1 = formatter.tparams(tparams1, (tp) => formatter.tparam(tp));
        ...
        const tparams2 = IArray.fromArray([createTypeParam('T'), createTypeParam('U')]);
        const result2 = formatter.tparams(tparams2, (tp) => formatter.tparam(tp));
    ...
    describe('sig', () => {
      test('formats function signatures correctly', () => {
        ...
        const params1 = IArray.fromArray([createFunParam('x', createTypeRef('number'))]);
        ...
        const sig4 = createFunSig(IArray.Empty, params1, createTypeRef('string'));
    ...
    describe('member', () => {
      test('formats TsMemberProperty correctly', () => {
        ...
        // Readonly property
        const member3 = createMemberProperty('name', createTypeRef('string'), TsProtectionLevel.default(), false, true);
        expect(formatter.member(member3)).toBe(' readonly name :string');

        // Static property
        const member4 = createMemberProperty('count', createTypeRef('number'), TsProtectionLevel.default(), true);
        expect(formatter.member(member4)).toBe('static  count :number');

        // Private property
        const member5 = createMemberProperty('_internal', createTypeRef('any'), TsProtectionLevel.private());
      ...
      test('formats TsMemberFunction correctly', () => {
        ...
        const params = IArray.fromArray([createFunParam('x', createTypeRef('number'))]);
        ...
        const sig2 = createFunSig(IArray.Empty, params, createTypeRef('string'));
        const member3 = createMemberFunction('convert', sig2);
        expect(formatter.member(member3)).toBe('convert (x : number): string');

        // Getter method
        const sig3 = createFunSig(IArray.Empty, IArray.Empty, createTypeRef('string'));
        const member4 = createMemberFunction('name', sig3, TsProtectionLevel.default(), MethodType.getter());
        expect(formatter.member(member4)).toBe('get name (): string');

        // Setter method
        const setterParams = IArray.fromArray([createFunParam('value', createTypeRef('string'))]);
        const sig4 = createFunSig(IArray.Empty, setterParams);
        const member5 = createMemberFunction('name', sig4, TsProtectionLevel.default(), MethodType.setter());
  ...
  describe('Type Formatting - Basic Types', () => {
    describe('apply', () => {
      test('formats basic types correctly', () => {
        ...
        const tpe3 = createTypeRef('Map', createTypeRef('string'), createTypeRef('number'));
        ...
        const tpe8 = TsTypeObject.create(Comments.empty(), IArray.fromArray([member as TsMember]));
      ...
      test('formats function types correctly', () => {
        ...
        // Function with return type
        const sig3 = createFunSig(IArray.Empty, params, createTypeRef('string'));
  ...
  describe('Type Formatting - Advanced Types', () => {
    describe('apply', () => {
      test('formats union and intersection types correctly', () => {
        ...
        const tpe1 = TsTypeUnion.create(IArray.fromArray([createTypeRef('string') as TsType, createTypeRef('number') as TsType]));
        expect(formatter.apply(tpe1)).toBe('string | number');

        // TsTypeUnion with multiple types
        const tpe2 = TsTypeUnion.create(IArray.fromArray([
        ...
        const tpe3 = TsTypeIntersect.create(IArray.fromArray([createTypeRef('A') as TsType, createTypeRef('B') as TsType]));
        expect(formatter.apply(tpe3)).toBe('A & B');

        // TsTypeIntersect with multiple types
        const tpe4 = TsTypeIntersect.create(IArray.fromArray([
      ...
      test('formats tuple types correctly', () => {
        ...
        const labeledElem = TsTupleElement.create(O.some(createSimpleIdent('name')), createTypeRef('string'));
      ...
      test('formats utility types correctly', () => {
        ...
        // TsTypeLookup
        const tpe2 = TsTypeLookup.create(createTypeRef('T'), createTypeRef('K'));
