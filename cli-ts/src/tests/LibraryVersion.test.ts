...
import { some, none, isSome, isNone, getOrElse } from 'fp-ts/Option';
import { pipe } from 'fp-ts/function';
import { LibraryVersion, InGit } from '../internal/LibraryVersion.js';
...
describe('LibraryVersion', () => {
  describe('Basic Construction and Property Access', () => {
    test('should construct with all parameters', () => {
      ...
      const libraryVersion1 = LibraryVersion.create(
        false,
        '1.2.3',
        null
      );
      ...
      expect(pipe(libraryVersion1.libraryVersion, getOrElse(() => ''))).toBe('1.2.3');
      expect(isNone(libraryVersion1.inGit)).toBe(true);

      // Test construction with stdlib flag
      const libraryVersion2 = LibraryVersion.create(
        true,
        '4.5.6',
        null
      );
      expect(libraryVersion2.isStdLib).toBe(true);
      expect(pipe(libraryVersion2.libraryVersion, getOrElse(() => ''))).toBe('4.5.6');
      ...
      const libraryVersion3 = LibraryVersion.create(
        false,
        '2.0.0',
        gitInfo
      );
      ...
      if (isSome(libraryVersion3.inGit)) {
        expect(libraryVersion3.inGit.value.repo.toString()).toBe('https://github.com/example/repo');
        expect(libraryVersion3.inGit.value.isDefinitelyTyped).toBe(false);
      }

      // Test construction with no library version
      const libraryVersion4 = LibraryVersion.create(
        false,
        null,
        null
      );
  ...
  describe('Version Generation with Digest', () => {
    test('should generate versions with digest correctly', () => {
      ...
      expect(version1.endsWith(testDigest.hexString.substring(0, 6))).toBe(true);
      ...
      expect(version2.endsWith(testDigest.hexString.substring(0, 6))).toBe(true);
      ...
      expect(version3.endsWith(testDigest.hexString.substring(0, 6))).toBe(true);
    });
  });
  ...
  describe('StdLib Version Handling', () => {
    test('should handle stdlib version truncation correctly', () => {
      ...
      const stdLibVersion = LibraryVersion.create(
        true,
        '4.5.6',
        null
      );
      ...
      const nonStdLibVersion = LibraryVersion.create(
        false,
        '4.5.6',
        null
      );
      ...
      const twoPartVersion = LibraryVersion.create(
        true,
        '4.5',
        null
      );
      ...
      const singlePartVersion = LibraryVersion.create(
        true,
        '4',
        null
      );
  ...
  describe('Edge Cases and Complex Scenarios', () => {
    test('should handle edge cases and complex scenarios', () => {
      ...
      const emptyVersionLib = LibraryVersion.create(
        false,
        '',
        null
      );
      ...
      expect(dtStdLibResult.endsWith(testDigest.hexString.substring(0, 6))).toBe(true);

      // Test digest hex string truncation
      const shortDigestTest = LibraryVersion.create(
        false,
        'test',
        null
      );
