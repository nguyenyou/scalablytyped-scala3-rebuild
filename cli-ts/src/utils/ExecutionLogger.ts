...
export class ExecutionLogger {
  ...
  constructor(logFilePath: string, workingDirectory: string, outputDirectory: string) {
   ...
   */
  static create(workingDirectory: string, outputDirectory: string): ExecutionLogger {
   ...
   */
  static withCustomPath(logFilePath: string, workingDirectory: string, outputDirectory: string): ExecutionLogger {
    return new ExecutionLogger(logFilePath, workingDirectory, outputDirectory);
  }
  ...
  async finalizeExecutionLog(success: boolean): Promise<void> {
    if (this.logWriter) {
      ...
      this.logWriter.write('\n');
      this.logWriter.write('='.repeat(50) + '\n');
      this.logWriter.write(`Execution ${success ? 'completed successfully' : 'failed'} at: ${endTime}\n`);
