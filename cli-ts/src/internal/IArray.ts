...
export const IArrayPatterns = {
	...
	exactlyFour: <T>(arr: IArray<T>): [T, T, T, T] | undefined =>
		arr.length === 4
			? [arr.apply(0), arr.apply(1), arr.apply(2), arr.apply(3)]
			: undefined,
...
export class IArray<T> {
	...
	minBy<U>(f: (value: T) => U, ordering: Ordering<U>): T {
		...
		let minValue: U | undefined;
		let minElem: T | undefined;
		let first = true;
	 ...
	 */
	mapNotNoneOption<U>(
		f: (value: T) => { _tag: "Some"; value: U } | { _tag: "None" },
	): IArray<U> {
		if (this.isEmpty) {
			return IArray.Empty;
		}
...
export function createIArrayOrdering<T>(
	elementOrdering: Ordering<T>,
): Ordering<IArray<T>> {
	return {
...
export function createIArrayEncoder<T>(
...
export function createIArrayDecoder<T>(
