...
export class Name {
	...
	static necessaryRewriteString(ident: string): string | undefined {
		...
		const patchedChars = Array.from(ident)
			.flatMap((char) => {
				switch (char) {
		...
		const fixedSpaces =
			"Space".repeat(initialSpaces.length) +
			patchedChars.slice(initialSpaces.length);

		// No kidding, instagram-private-api broke scalac with a stack overflow in the parser
		const notTooLong =
			fixedSpaces.length > 500 ? fixedSpaces.slice(0, 500) : fixedSpaces;
		...
		switch (notTooLong) {
			case "":
				legal = "_empty";
				break; // must have a name
			case "-":
				legal = "_dash";
				break; // `def `-`(d: Double) = d; `-`(d) doesn't do what you would think
			case "_":
				legal = "_underscore";
				break; // can't import a top level member with this name
			case "package":
				legal = "_package";
				break; // doesn't work
			default:
...
export class ScalaNameEscape {
	...
	private static isValidIdentifier(name: string): boolean {
		...
		return Array.from(name.slice(1)).every(
			(c) => c === "$" || /[\p{L}\p{Nl}\p{Nd}\p{Mn}\p{Mc}\p{Pc}]/u.test(c),
		);
	}
