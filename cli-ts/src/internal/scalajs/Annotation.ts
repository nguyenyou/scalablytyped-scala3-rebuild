...
export const Imported = {
	...
	isNamespace: (
		imported: Imported,
	): imported is { readonly _tag: "Namespace" } =>
	...
	isNamed: (
		imported: Imported,
	): imported is { readonly _tag: "Named"; readonly name: IArray<Name> } =>
		imported._tag === "Named",
};
...
export const Annotation = {
	...
	JsImport: (
		module: string,
		imported: Imported,
		global: Option<JsGlobalAnnotation> = O.none,
	): JsImportAnnotation => ({
		_tag: "JsImport",
		module,
		imported,
	 ...
	 */
	isJsBracketAccess: (
		annotation: AnnotationUnion,
	): annotation is JsBracketAccessAnnotation =>
	...
	isJsBracketCall: (
		annotation: AnnotationUnion,
	): annotation is JsBracketCallAnnotation =>
	...
	isScalaJSDefined: (
		annotation: AnnotationUnion,
	): annotation is ScalaJSDefinedAnnotation =>
	...
	isJsGlobalScope: (
		annotation: AnnotationUnion,
	): annotation is JsGlobalScopeAnnotation =>
	...
	isJsNameSymbol: (
		annotation: AnnotationUnion,
	): annotation is JsNameSymbolAnnotation => annotation._tag === "JsNameSymbol",
	 ...
	 */
	isLocationAnnotation: (
		annotation: AnnotationUnion,
	): annotation is LocationAnnotationUnion =>
		annotation._tag === "JsGlobalScope" ||
		annotation._tag === "JsImport" ||
		annotation._tag === "JsGlobal",
	...
	renamedFrom:
		(newName: Name) =>
		(oldAnnotations: IArray<AnnotationUnion>): IArray<AnnotationUnion> => {
			// Partition annotations into names and others
			// Equivalent to Scala's partition logic
			const { names, others } = pipe(oldAnnotations, (annotations) => {
				...
				for (let i = 0; i < annotations.length; i++) {
					...
					if (
						Annotation.isJsName(annotation) ||
			...
			const updatedNames: IArray<AnnotationUnion> = (() => {
				...
				if (
					names.isEmpty &&
					(newName.equals(Name.APPLY) || newName.equals(Name.namespaced))
				) {
				...
				if (names.isEmpty) {
					return IArray.fromArray([
						Annotation.JsName(newName),
					]) as IArray<AnnotationUnion>;
				}
	...
	getLocationAnnotations: (
		annotations: IArray<AnnotationUnion>,
	): IArray<LocationAnnotationUnion> => {
