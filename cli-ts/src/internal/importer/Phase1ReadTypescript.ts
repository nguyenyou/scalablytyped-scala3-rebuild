...
import { CalculateLibraryVersion } from './CalculateLibraryVersion';
...
export class Phase1ReadTypescript {
  ...
  apply(
    ...
    if (this.isIgnored(source) || isCircular) {
      logger.info(`Ignoring library ${source.libName.value} (ignored: ${this.isIgnored(source)}, circular: ${isCircular})`);
    ...
    try {
      ...
      logger.info(`Successfully created mock LibTs for ${source.libName.value}`);
   ...
   */
  private createMockParsedFile(source: LibTsSource, logger: Logger<void>): TsParsedFile {
...
export namespace Phase1ReadTypescript {
  ...
  export function createMockPipeline(
    ...
    logger.info(`Creating mock pipeline for ${libName.value} (React: ${involvesReact})`);
