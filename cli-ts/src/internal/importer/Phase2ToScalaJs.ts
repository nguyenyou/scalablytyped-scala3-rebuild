...
import { Versions, ScalaVersion } from './ConversionOptions';
...
export class Phase2ToScalaJs {
  ...
  apply(
    ...
    logger.info(`Converting TypeScript library to Scala.js: ${tsLibrary.name.value}`);
    ...
    try {
      ...
      return pipe(
        ...
        flatMap((scalaDeps: SortedMap<LibTsSource, LibScalaJs>) => {
          logger.info(`Processing ${tsLibrary.name.value} with ${scalaDeps.size} dependencies`);

          // DUMMY IMPLEMENTATION: Create mock Scala.js library
          const mockLibScalaJs = this.createMockLibScalaJs(source, tsLibrary, scalaDeps, logger);

          logger.info(`Successfully created mock LibScalaJs for ${tsLibrary.name.value}`);
      ...
      logger.error(`Failed to convert ${tsLibrary.name.value} to Scala.js: ${errorMessage}`);
   ...
   */
  private garbageCollectLibs(tsLibrary: LibTs, logger: Logger<void>): SortedSet<LibTsSource> {
   ...
   */
  private createMockPackageTree(scalaName: Name, logger: Logger<void>): PackageTree {
