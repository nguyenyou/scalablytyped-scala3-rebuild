...
import { type Comment, type Marker, Raw } from "./Comment.js";
import {
	type IArray,
...
function isMarker(comment: Comment): comment is Marker {
	return (
		"_markerTag" in comment && typeof (comment as any)._markerTag === "string"
	);
}
...
export class Comments {
	...
	get rawCs(): readonly string[] {
		return this.cs.filter(isRaw).map((raw) => raw.raw);
	}
	...
	extract<T>(pf: (marker: Marker) => T): Option<[T, Comments]> {
		...
		const remaining = [
			...this.cs.slice(0, foundIndex),
			...this.cs.slice(foundIndex + 1),
		];
	...
	has<_T extends Marker>(markerConstructor: any): boolean {
		...
		try {
			...
			return this.cs.some(
				(comment) => comment.constructor.name === constructorName,
			);
		}
	}
...
export namespace Comments {
	...
	export function format(comments: Comments): string {
		return comments.rawCs
			.map((raw) => {
				return formatComment(escapeUnicodeEscapes(escapeNestedComments(raw)));
			})
			.join("");
	}
	 ...
	 */
	export function formatWithFlag(
		comments: Comments,
