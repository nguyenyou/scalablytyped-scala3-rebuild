...
import { IArray, partialFunction, NumberOrdering } from '../IArray.js';
import {
  TsMember,
  TsMemberCall,
  TsMemberProperty,
...
export const DeriveNonConflictingName = {
  ...
  get isMeaningless(): Set<string> {
    return new Set([DeriveNonConflictingName.Anon, DeriveNonConflictingName.Fn]);
  },
  ...
  apply<T>(
    ...
    return (tryCreate: (name: TsIdentSimple) => Option<T>): T => {
      ...
      const fromCalls: Option<Detail> = (() => {
        const calls = members.collect(partialFunction<TsMember, TsMemberCall>(
        ...
        const longest = calls.maxBy((call) => call.signature.params.length, NumberOrdering);
      ...
      const fromMembers: IArray<Detail> = members.collect(partialFunction<TsMember, Detail>(
        (member) => member._tag === 'TsMemberProperty' || member._tag === 'TsMemberIndex' || member._tag === 'TsMemberFunction',
        (member) => {
          switch (member._tag) {
            ...
            case 'TsMemberIndex': {
              ...
              if (index.indexing._tag === 'IndexingSingle') {
                ...
                const short = Detail.pretty(single.name.parts.apply(single.name.parts.length - 1).value);
      ...
      )).sorted(Detail.compare).distinct();

      // Extract details from constructors
      const fromInstantiable: Option<Detail> = (() => {
        const result = members.collectFirst(partialFunction<TsMember, Detail>(
      ...
      const fromDict: Option<Detail> = (() => {
        const result = members.collectFirst(partialFunction<TsMember, Detail>(
          (member) => member._tag === 'TsMemberIndex' && (member as TsMemberIndex).indexing._tag === 'IndexingDict',
          (member) => {
            const index = member as TsMemberIndex;
      ...
      // Combine all details
      const allDetails = IArray.fromOptions(fromCalls, fromInstantiable, fromDict)
        .concat(fromMembers);

      // Generate name variants
      const nameVariants = DeriveNonConflictingName.generateNameVariants(prefix, allDetails);
   ...
   */
  generateNameVariants(prefix: string, details: IArray<Detail>): Generator<string> {
    return (function* () {
      const isMeaninglessPrefix = DeriveNonConflictingName.isMeaningless.has(prefix);
      ...
      for (const longVersion of [false, true]) {
        ...
        for (let amount = startAmount; amount <= details.length; amount++) {
          ...
          for (let idx = 0; idx < details.length; idx++) {
            ...
            const detailsPart = pick.map(detail => detail.pick(longVersion)).mkString('', '', '');
   ...
   */
  fallback<T>(prefix: string, tryCreate: (name: TsIdentSimple) => Option<T>): T {
    let idx = 0;
    while (true) {
