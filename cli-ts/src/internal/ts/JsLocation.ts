...
export const JsLocation = {
	 ...
	 */
	both: (
		module: JsLocationModule,
		global: JsLocationGlobal,
	): JsLocationBoth => ({
		_tag: "Both",
		module,
		global,
	...
	add: (location: JsLocation, ident: TsIdent): JsLocation => {
		...
		switch (location._tag) {
			...
			case "Module": {
				...
				return JsLocation.module(
					moduleLoc.module,
					ModuleSpec.add(moduleLoc.spec, ident),
				);
			}
			case "Both": {
				const bothLoc = location as JsLocationBoth;
				const newModule = JsLocation.add(
					bothLoc.module,
					ident,
				) as JsLocationModule;
				const newGlobal = JsLocation.add(
					bothLoc.global,
					ident,
				) as JsLocationGlobal;
		...
		if (TsDeclModule.isModule(tree)) {
			...
			if (!TsIdent.isNamespaced(namedDecl.name)) {
				return JsLocation.global(
					TsQIdent.append(location.jsPath, namedDecl.name),
				);
			}
		} else if (TsGlobal.isGlobal(tree)) {
			return JsLocation.zero();
	...
	navigateModule: (
		...
		if (TsDeclModule.isModule(tree)) {
			...
			return JsLocation.module(
				location.module,
				ModuleSpec.add(location.spec, namedDecl.name),
			) as JsLocationModule;
		}
		return location;
	},
	 ...
	 * Type guards
	 */
	isZero: (location: JsLocation): location is JsLocationZero =>
		location._tag === "Zero",
	isGlobal: (location: JsLocation): location is JsLocationGlobal =>
		location._tag === "Global",
	isModule: (location: JsLocation): location is JsLocationModule =>
		location._tag === "Module",
	isBoth: (location: JsLocation): location is JsLocationBoth =>
		location._tag === "Both",
};
