 ...
 */
export class TypeRewriter extends AbstractTreeTransformation<
	Map<TsType, TsType>
> {
		...
		if (key._tag === "TsTypeRef") {
			...
			if (typeRef.tparams.length === 0 && typeRef.name.parts.length === 1) {
				...
				if (namePart._tag === "TsIdentSimple") {
					...
					const found = tparams.find(
						(tp: TsTypeParam) => tp.name.value === simpleName.value,
					);
	 ...
	 */
	private transformFunSig(
		replacements: Map<TsType, TsType>,
		x: TsFunSig,
	): TsFunSig {
		// Transform result type if present
		const newResultType = x.resultType
			? pipe(
					x.resultType,
					fold(
						() => none,
