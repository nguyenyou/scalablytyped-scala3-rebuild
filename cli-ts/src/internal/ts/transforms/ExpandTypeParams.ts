 ...
 */
function createTypeRefPartialFunction(
	scope: TsTreeScope,
): PartialFunction<TsType, Either<TsTypeRef, TsTypeKeyOf>> {
	return {
		isDefinedAt: (tpe: TsType) => {
...
export class ExpandTypeParams extends TransformMembers {
			...
			switch (member._tag) {
				case "TsMemberCall": {
					...
					if (isSome(expandedSigs)) {
						...
						memberExpansion = RemoveComment.keepFirstOnly(
							newMembers,
							RemoveComment.r2,
	 ...
	 */
	private expandTParams(
		scope: TsTreeScope,
		sig: TsFunSig,
	): Option<IArray<TsFunSig>> {
		const expandables = sig.tparams.mapNotNoneOption((tp) =>
			this.expandable(scope, sig)(tp),
		);
		...
		const expanded = expandables.foldLeft(
			...
			(currentSigs: IArray<TsFunSig>, exp: ExpandableTypeParam) =>
				currentSigs.flatMap((currentSig) =>
					this.expandSignature(scope, exp)(currentSig),
				),
		);
	...
	private expandable(
		scope: TsTreeScope,
		sig: TsFunSig,
	): (tp: TsTypeParam) => Option<ExpandableTypeParam> {
		return (tp: TsTypeParam): Option<ExpandableTypeParam> => {
			...
			return pipe(tp.upperBound, (boundOpt) => {
				if (!isSome(boundOpt)) {
					return none;
				}
				...
				const [keyOfs, typeRefs, keepInBounds] = flattened.partitionCollect2(
					KeyOf,
					typeRefPF,
				);
				...
				if (
					distinctKeyOfs.length > 0 ||
					(filteredTypeRefs.length > 1 && isParam)
				) {
					const toKeepInBounds =
						distinctKeepInBounds.length > 0 ? some(distinctKeepInBounds) : none;
	 ...
	 */
	private expandSignature(
		scope: TsTreeScope,
		exp: ExpandableTypeParam,
	): (sig: TsFunSig) => IArray<TsFunSig> {
		return (sig: TsFunSig): IArray<TsFunSig> => {
			...
			const sigCleaned: TsFunSig = {
				...sig,
				tparams: sig.tparams.filterNot(
					(tp) => tp.name.value === exp.typeParam.value,
				),
			};
			...
			const expanded: IArray<TsFunSig> = exp.toExpand.flatMap((either) => {
				if (either._tag === "Left") {
					...
					const rewrites = new Map<TsType, TsType>([
						[
							TsTypeRef.fromIdent(exp.typeParam),
							this.clearCircularRef(exp.typeParam, tr),
						],
					]);

					const rewriter = new TypeRewriter(sigCleaned);
					return IArray.fromArray([
						rewriter.visitTsFunSig(rewrites)(sigCleaned),
					]);
				} else {
						...
						const pf: PartialFunction<TsMember, TsFunSig> = {
							...
							apply: (member: TsMember) => {
								...
								const rewrites = new Map<TsType, TsType>([
									[
										TsTypeRef.fromIdent(exp.typeParam),
										TsTypeLiteral.create(TsLiteral.str(n)),
									],
									[
										{
			...
			return pipe(keptInBounds, (keptOpt) =>
				isSome(keptOpt)
					? expanded.concat(IArray.fromArray([keptOpt.value]))
					: expanded,
			);
		};
	}
