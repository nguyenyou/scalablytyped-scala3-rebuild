...
export class ForwardCtors extends TreeTransformationScopedChanges {
	...
	private parentWithCtor(
		...
		return (parentRef: TsTypeRef): Option<IArray<TsMember>> => {
			...
			const parentName = parentRef.name.parts
				.toArray()
				.map((p) => p.value)
				.join(".");
			...
			const lookupResults = scope.lookupInternal(
				Picker.HasClassMemberss,
				parentRef.name.parts,
				newLd,
			);

			// Use firstDefined to find the first valid result
			const result = lookupResults.firstDefined(
				([parent, newScope]: [TsNamedDecl & HasClassMembers, TsTreeScope]) => {
					if (parent._tag === "TsDeclClass") {
						const parentClass = parent as TsDeclClass;

						// Fill in type parameters from the parent reference
						const parentRewritten = FillInTParams.apply(
							parentClass,
							parentRef.tparams,
						);
	...
	override enterTsDeclClass(
		...
		return (x: TsDeclClass): TsDeclClass => {
			...
			const ctors = this.parentWithCtor(
				scope,
