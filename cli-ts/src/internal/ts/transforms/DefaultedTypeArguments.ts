...
import { type TsType, type TsTypeParam, TsTypeRef } from "../trees.js";
...
export class DefaultedTypeArguments extends TreeTransformationScopedChanges {
	...
	enterTsTypeRef(scope: TsTreeScope): (typeRef: TsTypeRef) => TsTypeRef {
		return (typeRef: TsTypeRef) => {
			...
			for (const decl of typeDeclarations) {
				...
				if (
					expectedTparams._tag === "Some" &&
					expectedTparams.value.length !== providedTypeArgs.length
				) {
					...
					const newTParams = IArray.fromArray(
						expectedTparamsArray
							.toArray()
							.map((current: TsTypeParam, idx: number) => {
								if (idx < providedTypeArgs.length) {
									...
									const next = pipe(
										...
										getOrElse((): TsType => {
											...
											return pipe(
												...
												getOrElse(
													(): TsType =>
														TsTypeRef.any.withComments(
															Comments.apply([Comment.warning(msg)]),
														),
												),
											);
										}),
									...
									const next3 = this.instantiateTypeReferences(
										next2,
										instantiated,
									);
					...
					return TsTypeRef.create(typeRef.comments, typeRef.name, newTParams);
				}
			}
	 ...
	 */
	private handleSelfReference(
		next: TsType,
		originalTypeRef: TsTypeRef,
	): TsType {
	 ...
	 */
	private instantiateTypeReferences(
		type: TsType,
		instantiated: Map<TsType, TsType>,
	): TsType {
