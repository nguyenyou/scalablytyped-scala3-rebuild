 ...
 */
export const isProblems = <T>(
	res: Res<T>,
): res is { readonly _tag: "Problems"; readonly problems: IArray<Problem> } => {
	...
	isTypeMapping: (tpe: TsType): TsMemberTypeMapped | undefined => {
		if (tpe._tag === "TsTypeObject") {
			...
			if (
				...
				obj.members.get(0)?._tag === "TsMemberTypeMapped"
			) {
		...
		switch (resolvedKeys._tag) {
			case "TsTypeRef": {
				...
				if (isAbstract(scope, typeRef.name)) {
					return Res.Problems(
						IArray.apply(Problem.NotStatic(scope, typeRef) as Problem),
					);
				}
				...
				if (lookupResults.length === 0) {
					return Res.Problems(
						IArray.apply(Problem.TypeNotFound(scope, typeRef) as Problem),
					);
				}
			...
			case "TsTypeUnion": {
				...
				return Res.flatMap(Res.sequence(results), (keySets) => {
			...
			default:
				return Res.Problems(
					IArray.apply(
						Problem.NotKeysFromTarget(scope, resolvedKeys) as Problem,
					),
				);
		}
	};
...
function handleConditionalType(
	...
	if (
		conditional.ifTrue._tag === "TsTypeRef" &&
		...
		if (isEqual(extends_.tpe, conditional.ifFalse)) {
			...
			return Res.flatMap(evaluateKeys(scope, ld)(extends_.tpe), (leftKeys) =>
				Res.flatMap(evaluateKeys(scope, ld)(extends_.extends), (rightKeys) => {
	...
	if (
		conditional.ifFalse._tag === "TsTypeRef" &&
	...
	return Res.Problems(
		IArray.apply(Problem.NotKeysFromTarget(scope, conditional) as Problem),
	);
}
...
function handleTypeLookup(
	...
	return Res.Problems(
		IArray.apply(Problem.NotKeysFromTarget(scope, lookup) as Problem),
	);
}
...
function _evaluatePredicate(x: TsType): Res<boolean> {
	if (x._tag === "TsTypeExtends") {
		...
		if (
			extends_.extends._tag === "TsTypeRef" &&
...
export const AllMembersFor = {
	 ...
	 */
	forType:
		(scope: TsTreeScope, ld: LoopDetector) =>
		(tpe: TsType): Res<IArray<TsMember>> => {
			switch (tpe._tag) {
				case "TsTypeRef":
				...
				case "TsTypeIntersect": {
					...
					const results = intersect.types.map((t) =>
						AllMembersFor.forType(scope, ld)(t),
					);
					return Res.flatMap(Res.sequence(results), (memberArrays) => {
						const flattened = memberArrays.flatMap((arr) => arr);
						const hasTypeObject = intersect.types.exists(
							(t: TsType) => t._tag === "TsTypeObject",
						);
				...
				default: {
					...
					return Res.Problems(
						IArray.apply(Problem.InvalidType(scope, tpe) as Problem),
					);
				}
			}
		},
	...
	forInterface:
		(scope: TsTreeScope, ld: LoopDetector) =>
		(i: TsDeclInterface): Res<IArray<TsMember>> => {
			const parentResults = i.inheritance.map((parent) =>
				limitInlining(scope, parent, AllMembersFor.apply(scope, ld)(parent)),
			);

			return Res.flatMap(Res.sequence(parentResults), (fromParents) => {
				const combinedMembers = Utils.handleOverridingFields(
					i.members,
					fromParents,
				);
	 ...
	 */
	apply:
		(scope: TsTreeScope, _ld: LoopDetector) =>
		(typeRef: TsTypeRef): Res<IArray<TsMember>> => {
...
function handleTypeMapping(
	...
	if (tm.from._tag === "TsTypeRef") {
		...
		if (isAbstract(scope, fromRef.name)) {
			return Res.Problems(
				IArray.apply(Problem.NotStatic(scope, fromRef) as Problem),
			);
		}
	}

	// Evaluate keys from the 'from' type
	return Res.flatMap(evaluateKeys(scope, ld)(tm.from), (keys) => {
				...
				switch (x._tag) {
					...
					case "TsDeclTypeAlias": {
						...
						if (result._tag === "Problems") {
							...
							const keyResult = evaluateKeys(
								scope as any,
								LoopDetector.initial,
							)(ta.alias);

							if (keyResult._tag === "Ok" && keyResult.wasRewritten) {
								const notice = new Raw(
									`/* Inlined ${formatType(ta.alias)} */\n`,
								);
								const unionTypes = Array.from(keyResult.value).map(
									(key) => TsTypeLiteral.create(key.lit) as TsType,
								);
...
export class ExpandTypeMappingsAfter extends TreeTransformationScopedChanges {
	...
	private isTooDeep(scope: TsTreeScope): boolean {
		const numTypeObjects = scope.stack.filter(
			(tree: TsTree) => tree._tag === "TsTypeObject",
		).length;
