...
export class InferEnumTypes extends TreeTransformationScopedChanges {
	 ...
	 */
	enterTsDeclEnum(scope: TsTreeScope): (e: TsDeclEnum) => TsDeclEnum {
		return (e: TsDeclEnum) => {
	 ...
	 */
	private initializeMembers(members: IArray<TsEnumMember>): IArray<TsEnumMember> {
	 ...
	 */
	private replaceReferences(members: IArray<TsEnumMember>): IArray<TsEnumMember> {
	 ...
	 */
	private createMemberLookup(members: IArray<TsEnumMember>): Map<string, TsEnumMember> {
	 ...
	 * @returns The transformed expression
	 */
	private visitExprRecursive(expr: TsExpr, byName: Map<string, TsEnumMember>): TsExpr {
		switch (expr._tag) {
			case "TsExprRef":
			case "TsExprLiteral":
			...
			case "TsExprCall": {
				...
				const newParams = callExpr.params.map((p: TsExpr) => this.visitExpr(p, byName));
