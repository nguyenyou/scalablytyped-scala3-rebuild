...
class ResolveTypeQueriesVisitor extends TransformMembers {
	 ...
	 */
	newClassMembersLeaving(
		scope: TsTreeScope,
		tree: HasClassMembers,
	): IArray<TsMember> {
		return tree.members.flatMap((member: TsMember): IArray<TsMember> => {
			if (member._tag === "TsMemberProperty") {
				...
				if (
					...
					const note = Comment.create(
						`/* was \`${TsTypeFormatter.apply(tpe)}\` */\n`,
					);

					const founds = this.lookup(scope, Picker.NamedValues, tpe.expr).map(
						([found, newScope]) => {
							if (found._tag === "TsDeclVar") {
								...
								const resolvedType = this.typeOf(
									found,
									newScope,
					...
					if (founds.isEmpty) {
						...
						const warningType = TsTypeRef.any.withComments(
							Comments.fromComment(Comment.warning(msg)),
						);
						return IArray.apply(
	...
	newMembers(scope: TsTreeScope, tree: TsContainer): IArray<TsContainerOrDecl> {
		...
		for (const stackItem of scope.stack) {
			if (
				stackItem &&
				typeof stackItem === "object" &&
				"codePath" in stackItem
			) {
		...
		const rewritten = tree.members.flatMap(
			(member: TsContainerOrDecl): IArray<TsContainerOrDecl> => {
				if (member._tag === "TsDeclVar") {
					...
					if (
						...
						const ownerLoc =
							"jsLocation" in tree
								? (tree as any).jsLocation
								: JsLocation.zero();

						const founds = this.lookup(
							scope,
							Picker.NamedValues,
							tpe.expr,
						).flatMap(([found, _newScope]) => {
							...
							const derivedDecls = this.deriveCopy(
								found.addComment(note),
								tree.codePath,
								some(target.name),
							);

							// Apply JS location to derived declarations
							const _setJsLocation = new SetJsLocation();
							return derivedDecls.map((decl) => {
								if (
									"withJsLocation" in decl &&
									typeof decl.withJsLocation === "function"
								) {
	...
	private deriveCopy(
		...
		const newName: TsIdentSimple = isSome(rename)
			? rename.value
			: decl.name._tag === "TsIdentSimple"
				? (decl.name as TsIdentSimple)
				: TsIdent.simple(decl.name.value);

		// For now, just return a copy with the new name and code path
		const newCodePath =
			ownerCp._tag === "HasPath"
		...
		switch (decl._tag) {
			case "TsDeclVar":
				return IArray.apply(
					(decl as TsDeclVar)
						.withName(newName)
						.withCodePath(newCodePath) as TsNamedDecl,
				);
			case "TsDeclFunction":
				return IArray.apply(
					(decl as TsDeclFunction)
						.withName(newName)
						.withCodePath(newCodePath) as TsNamedDecl,
				);
			case "TsDeclClass":
	...
	private lookup<T extends TsNamedDecl>(
		...
		let results = scope.lookupInternal(
			picker,
			wanted.parts,
			LoopDetector.initial,
		);
		...
		if (results.isEmpty) {
			...
			const patchedWanted =
				wanted.parts.length > 0 &&
				wanted.parts.apply(0)._tag === "TsIdentLibrarySimple"
					? IArray.fromArray([
							wanted.parts.apply(0),
							TsIdentGlobal,
							...wanted.parts.drop(1).toArray(),
						])
					: IArray.fromArray([TsIdentGlobal, ...wanted.parts.toArray()]);

			results = scope.lookupInternal(
				picker,
				patchedWanted,
				LoopDetector.initial,
			);
	...
	private resolve(
		...
		if (this.qidentEquals(target.expr, GlobalThis)) {
			return TsTypeRef.any.withComments(
				Comments.fromComment(Comment.create("/* globalThis */ ")),
			);
		}
		...
		if (functions.isEmpty) {
			...
			functions.length === 1 &&
			functions.apply(0).signature.tparams.isEmpty &&
			rest.length <= 1
		) {
	 ...
	 */
	private nonEmptyTypeObject(
		from: TsDeclNamespaceOrModule,
	): Option<TsTypeObject> {
		const rewritten = from.members.mapNotNone(
			(member: TsContainerOrDecl): TsMember | undefined => {
				switch (member._tag) {
					case "TsDeclNamespace": {
						const ns = member as TsDeclNamespace;
						const nestedType = this.nonEmptyTypeObject(ns);
						return isSome(nestedType)
							? (TsMemberProperty.create(
									ns.comments,
									TsProtectionLevel.default(),
									ns.name,
