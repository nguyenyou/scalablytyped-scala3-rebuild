...
import type { TsType, TsTypeRef, TsQIdent, TsDeclEnum, TsEnumMember } from "@/internal/ts/trees.js";
...
export class InlineConstEnum extends TreeTransformationScopedChanges {
	 ...
	 */
	private tryInlineEnumMember(scope: TsTreeScope, parts: IArray<any>): TsType | undefined {
		...
		const enumQIdent: TsQIdent = {
			_tag: "TsQIdent",
			parts: enumParts,
			asString: `TsQIdent(${enumParts.toArray().map(p => p.value || String(p)).join(".")})`,
		};
			...
			if (enumDecl._tag === "TsDeclEnum") {
				...
				if (declEnum.isConst) {
					...
					if (member !== undefined) {
						...
						scope.logger.info(`Inlining const enum type ${originalTypeStr} => ${inlinedTypeStr}`);
	 ...
	 */
	private findEnumMember(enumDecl: TsDeclEnum, memberName: any): TsEnumMember | undefined {
