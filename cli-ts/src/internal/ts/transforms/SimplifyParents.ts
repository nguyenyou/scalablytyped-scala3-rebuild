...
import { Picker } from '../TsTreeScope.js';
import { some, none } from 'fp-ts/Option';

/**
 * Helper function to create a proper TsTypeRef object with all required methods
 */
function createTsTypeRef(name: TsQIdent, tparams: IArray<TsType> = IArray.Empty, comments: Comments = Comments.empty()): TsTypeRef {
  return {
    _tag: 'TsTypeRef',
...
export class SimplifyParents extends TreeTransformationScopedChanges {
  ...
  override enterTsDeclClass(scope: TsTreeScope): (x: TsDeclClass) => TsDeclClass {
    return (x: TsDeclClass) => {
      ...
      const allParents = IArray.fromArray(parentArray).concat(x.implementsInterfaces);
   ...
   */
  private newParents(parents: IArray<TsTypeRef>, scope: TsTreeScope): IArray<TsTypeRef> {
   ...
   */
  private lift(scope: TsTreeScope, ref: TsTypeRef, tpe: TsType): IArray<TsTypeRef> {
    switch (tpe._tag) {
      case 'TsTypeRef': {
        ...
        if (lookup.length > 0) {
          ...
          if (codePath && codePath._tag === 'HasPath') {
            scope.logger.info(`Simplified class which extends var ${ref.asString} to typeof var`);
            ...
            return IArray.fromArray([
              createTsTypeRef(hasPath.codePathPart, typeRef.tparams, typeRef.comments)
            ]);
          }
        }
        ...
        for (const [decl, _] of classLookup.toArray()) {
          if (decl._tag === 'TsDeclClass') {
            ...
            if (codePath && codePath._tag === 'HasPath') {
              ...
              return IArray.fromArray([
                createTsTypeRef(hasPath.codePath)
              ]);
            } else {
              // Fallback to using the query expression as the name
              return IArray.fromArray([
                createTsTypeRef(query.expr)
              ]);
            }
          }
        }
