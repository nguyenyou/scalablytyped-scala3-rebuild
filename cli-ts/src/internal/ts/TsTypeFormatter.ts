...
class TsTypeFormatterImpl implements TsTypeFormatter {
	...
	private formatOptionalModifier(
	...
	apply(tpe: TsType): string {
		switch (tpe._tag) {
			case "TsTypeRef": {
				...
				const commentsStr = this.keepComments
					? Comments.format(typeRef.comments)
					: "";
			...
			case "TsTypeObject": {
				...
				const commentsStr = this.keepComments
					? Comments.format(obj.comments)
					: "";
				const membersStr = obj.members
					.map((m) => this.member(m))
					.toArray()
					.join(", ");
			...
			case "TsTypeTuple": {
				...
				const elementsStr = tuple.elems
					.map((e) => this.tupleElement(e))
					.toArray()
					.join(", ");
				return `[${elementsStr}]`;
			}
 ...
 */
export const TsTypeFormatterNoComments: TsTypeFormatter =
	new TsTypeFormatterImpl(false);
