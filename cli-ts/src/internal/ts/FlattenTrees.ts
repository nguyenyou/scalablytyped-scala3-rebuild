...
export const FlattenTrees = {
  ...
  apply(files: IArray<TsParsedFile>): TsParsedFile {
    return files.foldLeft(FlattenTrees.EmptyFile, (acc, file) => FlattenTrees.mergeFile(acc, file));
  },
  ...
  mergeComments(one: Comments, two: Comments): Comments {
    if (one.cs.length === two.cs.length &&
   ...
   */
  newMembers(these: IArray<TsContainerOrDecl>, thats: IArray<TsContainerOrDecl>): IArray<TsContainerOrDecl> {
    // Partition into named and unnamed declarations
    const [theseNamedDecls, theseUnnamedDecls] = these.partitionCollect(partialFunction(
      (x: TsContainerOrDecl): x is TsNamedDecl => TsTree.isNamedDecl(x),
      (x: TsContainerOrDecl) => x as TsNamedDecl
    ));

    const [thatsNamedDecls, thatsUnnamedDecls] = thats.partitionCollect(partialFunction(
      (x: TsContainerOrDecl): x is TsNamedDecl => TsTree.isNamedDecl(x),
      (x: TsContainerOrDecl) => x as TsNamedDecl
    ));

    const rets = IArrayBuilder.fromIArray<TsContainerOrDecl>(theseUnnamedDecls, these.length + thats.length);
        ...
        rets.addOrUpdateMatching(
          ...
          (x: TsContainerOrDecl) => {
            if (TsGlobal.isGlobal(x)) {
              ...
              return xGlobal.withMembers(FlattenTrees.newMembers(IArray.Empty, xGlobal.members));
            }
            return x;
          },
          partialFunction(
            (existing: TsContainerOrDecl): existing is TsGlobal => TsGlobal.isGlobal(existing),
            (existing: TsContainerOrDecl) => FlattenTrees.mergeGlobal(thatGlobal, existing as TsGlobal)
          )
        );
      } else {
        rets.addOne(that);
    ...
    FlattenTrees.newNamedMembers(theseNamedDecls, thatsNamedDecls).forEach(member => {
      rets.addOne(member);
    });
      ...
      if (TsDeclNamespace.isNamespace(that)) {
        ...
        rets.addOrUpdateMatching(
          ...
          (x: TsNamedDecl) => {
            if (TsDeclNamespace.isNamespace(x)) {
              ...
              return xNamespace.withMembers(FlattenTrees.newMembers(IArray.Empty, xNamespace.members));
            }
            return x;
          },
          partialFunction(
            (existing: TsNamedDecl): boolean => {
              if (TsDeclNamespace.isNamespace(existing)) {
                ...
                return thatNamespace.name.value === existingNamespace.name.value;
            ...
            (existing: TsNamedDecl) => {
              if (TsDeclNamespace.isNamespace(existing)) {
                return FlattenTrees.mergeNamespaces(thatNamespace, existing as TsDeclNamespace);
              } else if (TsDeclFunction.isFunction(existing)) {
                return FlattenTrees.mergeNamespaceAndFunction(thatNamespace, existing as TsDeclFunction);
              } else if (TsDeclVar.isVar(existing)) {
                return FlattenTrees.mergeNamespaceAndVar(thatNamespace, existing as TsDeclVar);
              }
              return existing;
            }
          )
        ...
        rets.addOrUpdateMatching(
          ...
          partialFunction(
            ...
            (existing: TsNamedDecl) => {
              if (TsDeclNamespace.isNamespace(existing)) {
                return FlattenTrees.mergeNamespaceAndFunction(existing as TsDeclNamespace, thatFunction);
              }
              return existing;
            }
          )
   ...
   */
  handleOtherNamedDeclarations(rets: IArrayBuilder<TsNamedDecl>, that: TsNamedDecl): void {
    if (TsDeclModule.isModule(that)) {
      ...
      rets.addOrUpdateMatching(
        ...
        (x: TsNamedDecl) => {
          if (TsDeclModule.isModule(x)) {
            ...
            return xModule.withMembers(FlattenTrees.newMembers(IArray.Empty, xModule.members));
          }
          return x;
        },
        partialFunction(
          ...
          (existing: TsNamedDecl) => {
            if (TsDeclModule.isModule(existing)) {
              return FlattenTrees.mergeModule(thatModule, existing as TsDeclModule);
            }
            return existing;
          }
        )
      ...
      rets.addOrUpdateMatching(
        ...
        partialFunction(
          ...
          (existing: TsNamedDecl) => {
            if (TsDeclClass.isClass(existing)) {
              return FlattenTrees.mergeClass(thatClass, existing as TsDeclClass);
            } else if (TsDeclInterface.isInterface(existing)) {
              return FlattenTrees.mergedClassAndInterface(thatClass, existing as TsDeclInterface);
            }
            return existing;
          }
        )
   ...
   */
  handleRemainingNamedDeclarations(rets: IArrayBuilder<TsNamedDecl>, that: TsNamedDecl): void {
    if (TsDeclInterface.isInterface(that)) {
      ...
      rets.addOrUpdateMatching(
        ...
        partialFunction(
          ...
          (existing: TsNamedDecl) => {
            if (TsDeclInterface.isInterface(existing)) {
              return FlattenTrees.mergeInterface(thatInterface, existing as TsDeclInterface);
            } else if (TsDeclClass.isClass(existing)) {
              return FlattenTrees.mergedClassAndInterface(existing as TsDeclClass, thatInterface);
            }
            return existing;
          }
        )
          ...
          (existing: TsNamedDecl) => {
            if (TsDeclVar.isVar(existing)) {
              ...
              return TsDeclVar.create(
                FlattenTrees.mergeComments(existingVar.comments, thatVar.comments),
              ...
              return FlattenTrees.mergeNamespaceAndVar(existing as TsDeclNamespace, thatVar);
            }
            return existing;
          }
        )
   ...
   */
  handleFinalNamedDeclarations(rets: IArrayBuilder<TsNamedDecl>, that: TsNamedDecl): void {
      ...
      rets.addOrUpdateMatching(
        ...
        (x: TsNamedDecl) => {
          if (TsAugmentedModule.isAugmentedModule(x)) {
            ...
            return xAugmented.withMembers(FlattenTrees.newMembers(IArray.Empty, xAugmented.members));
          }
          return x;
        },
        partialFunction(
          ...
          (existing: TsNamedDecl) => {
            if (TsAugmentedModule.isAugmentedModule(existing)) {
              return FlattenTrees.mergeAugmentedModule(thatAugmented, existing as TsAugmentedModule);
            }
            return existing;
          }
        )
            ...
            if (TsDeclTypeAlias.isTypeAlias(existing)) {
              ...
              if (nonTrivialCandidates.length === 1) {
                ...
                return TsDeclTypeAlias.create(
                  ...
                  FlattenTrees.mergeTypeParams(existingTypeAlias.tparams, thatTypeAlias.tparams),
                  TsTypeIntersect.simplified(IArray.fromArray([existingTypeAlias.alias, thatTypeAlias.alias])),
                  FlattenTrees.mergeCodePath(existingTypeAlias.codePath, thatTypeAlias.codePath)
                );
              }
            }
            return existing;
   ...
   */
  newClassMembers(these: IArray<TsMember>, thats: IArray<TsMember>): IArray<TsMember> {
          ...
          partialFunction(
            (existing: TsMember): boolean => {
              if (TsMemberProperty.isMemberProperty(existing)) {
                ...
                return thatProperty.name.value === existingProperty.name.value &&
            ...
            (existing: TsMember) => {
              if (TsMemberProperty.isMemberProperty(existing)) {
                ...
                return TsMemberProperty.create(
                  FlattenTrees.mergeComments(existingProperty.comments, thatProperty.comments),
                  existingProperty.level,
                  existingProperty.name,
                  FlattenTrees.bothTypes(existingProperty.tpe, thatProperty.tpe),
          ...
          partialFunction(
            (existing: TsMember): boolean => {
              if (TsMemberIndex.isMemberIndex(existing)) {
                ...
                return FlattenTrees.indexingEquals(thatIndex.indexing, existingIndex.indexing);
              }
              return false;
            },
            (existing: TsMember) => {
              if (TsMemberIndex.isMemberIndex(existing)) {
                ...
                const mergedValueType = pipe(
                  IArray.fromOptions(existingIndex.valueType, thatIndex.valueType),
                  types => types.isEmpty ? none : some(TsTypeIntersect.simplified(types))
                );
                return TsMemberIndex.create(
                  FlattenTrees.mergeComments(existingIndex.comments, thatIndex.comments),
  ...
  indexingEquals(one: Indexing, two: Indexing): boolean {
    ...
    if (one._tag === 'IndexingDict' && two._tag === 'IndexingDict') {
      ...
      return oneDict.name.value === twoDict.name.value &&
    ...
    if (types.isEmpty) {
      ...
      const preferred = types.filter(tpe => {
        ...
        const neverTypes = TsTreeTraverse.collect(tpe, (tree: TsTree) => {
          if (tree._tag === 'TsTypeRef' && (tree as any).name?.value === 'never') {
            return tree;
          }
          return undefined;
        });
      ...
      return preferred.headOption ? some(preferred.headOption) : (types.headOption ? some(types.headOption) : none);
    }
  },
   ...
   */
  mergeTypeParams(thats: IArray<TsTypeParam>, existings: IArray<TsTypeParam>): IArray<TsTypeParam> {
   ...
   */
  mergeAugmentedModule(that: TsAugmentedModule, existing: TsAugmentedModule): TsAugmentedModule {
   ...
   */
  mergeInterface(that: TsDeclInterface, existing: TsDeclInterface): TsDeclInterface {
   ...
   */
  mergeNamespaces(that: TsDeclNamespace, existing: TsDeclNamespace): TsDeclNamespace {
   ...
   */
  mergeNamespaceAndFunction(ns: TsDeclNamespace, func: TsDeclFunction): TsDeclNamespace {
   ...
   */
  mergeNamespaceAndVar(ns: TsDeclNamespace, variable: TsDeclVar): TsDeclNamespace {
   ...
   */
  mergedClassAndInterface(cls: TsDeclClass, iface: TsDeclInterface): TsDeclClass {
    return TsDeclClass.create(
