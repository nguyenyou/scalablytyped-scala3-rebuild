...
export const TypeParamsReferencedInTree = {
	 ...
	 */
	apply: (
		inScope: Map<TsIdent, TsTypeParam>,
		tree: TsTree,
	): IArray<TsTypeParam> => {
		...
		const traverseWithScope = (
			node: TsTree,
			currentScope: Map<TsIdent, TsTypeParam>,
		) => {
		...
		const fromBounds = TsTreeTraverse.collectIArray(
			IArray.fromArray(bounds),
			(tree: TsTree) => {
		...
		const allReferencedIdents: IArray<TsIdent> = referencedInTree
			.appendedAll(fromBounds)
			.distinct();
