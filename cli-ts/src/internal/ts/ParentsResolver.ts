...
export function isInterfaceOrClass(tree: TsTree): tree is InterfaceOrClass {
	return (
		(tree._tag === "TsDeclClass" || tree._tag === "TsDeclInterface") &&
		"members" in tree
	);
}
...
export const ParentsResolver = {
	 ...
	 */
	apply: <X extends InterfaceOrClass>(
		scope: TsTreeScope,
		tree: X,
	): WithParents<X> => {
		...
		function innerRecurse(
			...
			if (found.isEmpty) {
				unresolvedBuilder.addOne(
					TsTypeRef.create(Comments.empty(), qualifiedName, currentTParams),
				);
				return;
			}
		...
		function outerRecurse(
			...
			if (isTsDeclInterface(currentTree)) {
				...
				const parentArray =
					cls.parent._tag === "Some"
						? IArray.apply(cls.parent.value)
						: IArray.Empty;
