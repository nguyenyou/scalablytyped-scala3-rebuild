...
import { TsIdentApply, TsIdentConstructor } from "./trees.js";
...
export const MemberCache = {
	...
	create: (members: IArray<TsContainerOrDecl>): MemberCache => {
		...
		const isModule =
			exports.nonEmpty ||
			imports.exists((imp) => imp.from._tag !== "TsImporteeLocal");
		...
		const modulesBase = nameds.collect(
			partialFunction(
				(m: TsNamedDecl): m is TsDeclModule => m._tag === "TsDeclModule",
				(m: TsNamedDecl) =>
					[m.name, m as TsDeclModule] as [TsIdentModule, TsDeclModule],
			),
		);
		const modules = modulesBase.toMap();

		// Extract augmented modules
		const directAugmented = nameds.collect(
			partialFunction(
				(m: TsNamedDecl): m is TsAugmentedModule =>
					m._tag === "TsAugmentedModule",
		...
		const augmentedModulesStringMap = new Map<
			string,
		...
		const augmentedModulesMap = new Map<
			TsIdentModule,
			IArray<TsAugmentedModule>
		>();
