 ...
 */
type TsTypeParameterizable = TsDeclInterface | TsDeclClass | TsDeclTypeAlias | TsFunSig;
...
export const FillInTParams = {
   ...
   */
  apply<T extends TsTypeParameterizable>(x: T, providedTParams: IArray<TsType>): T {
    return pipe(
      ...
      fold(
        ...
        (replacements) => {
          ...
          if (x._tag === 'TsDeclInterface') {
            const transformed = typeRewriter.visitTsDeclInterface(replacements)(x as TsDeclInterface);
            return {
              ...transformed,
              tparams: IArray.Empty
            } as T;
          } else if (x._tag === 'TsDeclClass') {
            const transformed = typeRewriter.visitTsDeclClass(replacements)(x as TsDeclClass);
            return {
              ...transformed,
              tparams: IArray.Empty
            } as T;
          } else if (x._tag === 'TsDeclTypeAlias') {
            const transformed = typeRewriter.visitTsDeclTypeAlias(replacements)(x as TsDeclTypeAlias);
            return {
              ...transformed,
              tparams: IArray.Empty
            } as T;
          } else if (x._tag === 'TsFunSig') {
            const transformed = typeRewriter.visitTsFunSig(replacements)(x as TsFunSig);
            return {
              ...transformed,
              tparams: IArray.Empty
            } as T;
    ...
    function recursiveBound(name: TsIdent, bound: TsType): boolean {
      const collected = TsTreeTraverse.collect(bound, (tree) => {
        if (tree._tag === 'TsTypeRef') {
          ...
          if (typeRef.name.parts.length === 1 && typeRef.name.parts.get(0) === name) {
            return name;
          }
        }
        return undefined;
  ...
  rewriter(
    ...
    for (let i = 0; i < expectedTParams.length; i++) {
      ...
      const provided = i < providedTParams.length
        ? providedTParams.get(i)
        : pipe(
            tp.default,
            getOrElse((): TsType => {
              const warningComment = Comment.warning(`${tp.name.value} not provided`);
              return {
                ...TsTypeRef.any,
