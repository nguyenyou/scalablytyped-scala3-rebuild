...
export const AllMembersFor = {
	 ...
	 */
	forType:
		(scope: TsTreeScope, loopDetector: LoopDetector) =>
		(tpe: TsType): IArray<TsMember> => {
			switch (tpe._tag) {
				case "TsTypeRef":
				...
				case "TsTypeIntersect": {
					...
					return intersectType.types.flatMap(
						AllMembersFor.forType(scope, loopDetector),
					);
				}
	...
	handleOverridingFields: (
		fromThis: IArray<TsMember>,
		fromParents: IArray<TsMember>,
	): IArray<TsMember> => {
		...
		const nonOverriddenParentProperties = parentProperties.filter(
			(prop) => !thisFieldOverrides.has(prop.name.value),
		);

		// Combine: this members + non-overridden parent properties + other parent members
		return fromThis.concat(nonOverriddenParentProperties).concat(parentRest);
	},
	...
	apply:
		(scope: TsTreeScope, loopDetector: LoopDetector) =>
		(typeRef: TsTypeRef): IArray<TsMember> => {
			...
			return lookupResults.flatMap(([decl, newScope]) => {
				switch (decl._tag) {
					case "TsDeclInterface":
						return AllMembersFor.forInterface(
							newLoopDetector,
							decl as TsDeclInterface,
							newScope,
							typeRef.tparams,
						);
					...
					case "TsDeclClass": {
						...
						const parentMembers = pipe(
							...
							(parents) =>
								parents.concat(filledClass.implementsInterfaces.toArray()),
							(allParents) => IArray.fromArray(allParents),
							(allParents) =>
								allParents.flatMap(
									AllMembersFor.apply(newScope, newLoopDetector),
								),
						);

						return AllMembersFor.handleOverridingFields(
							filledClass.members,
							parentMembers,
						);
					}
