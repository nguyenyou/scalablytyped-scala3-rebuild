...
export const Directive = {
  ...
  fromString: (directiveString: string): Option<Directive> => {
    ...
    // PathRef pattern
    const pathMatch = trimmed.match(/\/\/\/\s*<reference\s+path="([^"]+)"\s*\/>/);
    if (pathMatch) {
      return some(Directive.pathRef(pathMatch[1]));
    }
    
    // TypesRef pattern
    const typesMatch = trimmed.match(/\/\/\/\s*<reference\s+types="([^"]+)"\s*\/>/);
    if (typesMatch) {
    ...
    // AmdModule pattern
    const amdMatch = trimmed.match(/\/\/\/\s*<amd-module\s+name="([^"]+)"\s*\/>/);
    if (amdMatch) {
