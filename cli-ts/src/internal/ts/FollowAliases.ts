...
import { TsTypeRef, TsTypeUnion, TsTypeIntersect } from './trees.js';
...
export const FollowAliases = {
   ...
   */
  apply: (scope: TsTreeScope, skipValidation: boolean = false) => (tpe: TsType): TsType => {
    try {
      switch (tpe._tag) {
        case 'TsTypeRef': {
            ...
            partialFunction<TsNamedDecl, TsType>(
              ...
              (decl: TsNamedDecl): boolean => {
                ...
                if (decl._tag === 'TsDeclInterface') {
                  ...
                  return iface.inheritance.length === 1 && iface.members.length === 0;
                }
                return false;
              },
              ...
              (decl: TsNamedDecl): TsType => {
                if (decl._tag === 'TsDeclTypeAlias') {
                  ...
                  const filledInterface = FillInTParams.apply(i, typeRef.tparams);
                  return FollowAliases.apply(scope, skipValidation)(filledInterface.inheritance.apply(0));
                }
        ...
        case 'TsTypeIntersect': {
          ...
          const resolvedTypes = intersectType.types.map(t => FollowAliases.apply(scope, skipValidation)(t));
          return TsTypeIntersect.simplified(resolvedTypes);
        }

        case 'TsTypeUnion': {
          const unionType = tpe as TsTypeUnion;
          const resolvedTypes = unionType.types.map(t => FollowAliases.apply(scope, skipValidation)(t));
      ...
      if (error instanceof RangeError && error.message.includes('Maximum call stack')) {
        ...
        scope.logger.error(`Recovered from SOE while following type alias ${formatted}`);
   ...
   */
  typeRef: (scope: TsTreeScope) => (tr: TsTypeRef): TsTypeRef => {
        ...
        ([decl, newScope]: [TsNamedDecl, TsTreeScope]): TsTypeRef => {
          if (decl._tag === 'TsDeclTypeAlias') {
            ...
            if (filledAlias._tag === 'TsTypeRef') {
              return FollowAliases.typeRef(newScope)(filledAlias as TsTypeRef);
            }
            ...
            return FollowAliases.typeRef(newScope)(filledInterface.inheritance.apply(0));
          }
          // This should never happen due to the predicate, but TypeScript needs it
          return tr;
        }
