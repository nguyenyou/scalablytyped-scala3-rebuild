...
export const CodePath = {
	...
	hasPath: (inLibrary: TsIdent, codePathPart: TsQIdent): CodePathHasPath => {
		...
		return {
			...
			codePath,
			add: (ident: TsIdent) =>
				CodePath.hasPath(inLibrary, TsQIdent.append(codePathPart, ident)),
			...
			navigate: (tree: TsTree) => {
				...
				if (isNamedDecl(tree)) {
					...
					return CodePath.hasPath(
						inLibrary,
						TsQIdent.append(codePathPart, tree.name),
					);
				} else if (isGlobal(tree)) {
					// For TsGlobal, add the Global identifier
					return CodePath.hasPath(
						inLibrary,
						TsQIdent.append(codePathPart, TsIdentGlobal),
					);
				} else {
	...
	isHasPath: (path: CodePath): path is CodePathHasPath =>
		path._tag === "HasPath",
};
