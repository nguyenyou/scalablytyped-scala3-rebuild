...
export const TsQIdentUtils = {
  ...
  Primitive: (qident: TsQIdent): boolean => {
    ...
    return ['any', 'boolean', 'number', 'string', 'symbol', 'object', 'undefined', 'null', 'void', 'never', 'unknown', 'bigint'].includes(name);
  },
...
export const TsTreeScopeUtils = {
   ...
   */
  createAlternativeModuleName: (modName: TsIdentModule): TsIdentModule | undefined => {
...
export class LoopDetector {
   ...
   */
  includingTypeRef(typeRef: TsTypeRef, scope: TsTreeScope): Either<void, LoopDetector> {
   ...
   * Returns Left(unit) if this would create a loop, Right(newDetector) otherwise.
   */
  including(idents: IArray<TsIdent>, scope: TsTreeScope): Either<void, LoopDetector>;
  including(typeRef: TsTypeRef, scope: TsTreeScope): Either<void, LoopDetector>;
  including(identsOrTypeRef: IArray<TsIdent> | TsTypeRef, scope: TsTreeScope): Either<void, LoopDetector> {
    let entry: LoopDetectorEntry;

    if ('_tag' in identsOrTypeRef && identsOrTypeRef._tag === 'TsTypeRef') {
      // It's a TsTypeRef
      entry = LoopDetectorEntry.fromTypeRef(identsOrTypeRef as TsTypeRef, scope);
    } else {
      // It's an IArray<TsIdent>
      entry = LoopDetectorEntry.fromIdents(identsOrTypeRef as IArray<TsIdent>, scope);
    }
...
export const Picker = {
  Types: {
    pick: (decl: TsNamedDecl): Option<TsNamedDecl> => {
      if (decl._tag === 'TsDeclInterface' || decl._tag === 'TsDeclClass' || decl._tag === 'TsDeclTypeAlias' || decl._tag === 'TsDeclEnum') {
...
export interface TsTreeScope {
   ...
   */
  lookupIncludeScope(qname: TsQIdent, skipValidation?: boolean): IArray<[TsNamedDecl, TsTreeScope]>;
   ...
   */
  lookupTypeIncludeScope(qname: TsQIdent, skipValidation?: boolean): IArray<[TsNamedDecl, TsTreeScope]>;
...
export namespace TsTreeScope {
  ...
  export class Root implements TsTreeScope {
    private _depScopes: Map<TsIdentLibrary, [TsLib, TsParsedFile, Scoped]> | undefined;
    ...
    get stack(): TsTree[] { return []; }
    get tparams(): Map<TsIdent, TsTypeParam> { return new Map(); }
    get tkeys(): Set<TsIdent> { return new Set(); }
    get parent(): TsTreeScope { return this; }
    get exports(): IArray<TsExport> { return IArray.Empty; }

    /**
     * Lazy-computed dependency scopes
     */
    private get depScopes(): Map<TsIdentLibrary, [TsLib, TsParsedFile, Scoped]> {
      if (!this._depScopes) {
     ...
     */
    caching(): Root {
      return new Root(this.libName, this.pedantic, this._deps, this.logger, some(createCache()), this.lookupUnqualified);
    }
    ...
    enableUnqualifiedLookup(): Root {
      return new Root(this.libName, this.pedantic, this._deps, this.logger, this.cache, true);
    }
    ...
    lookup(qname: TsQIdent, skipValidation = false): IArray<TsNamedDecl> {
      return this.lookupIncludeScope(qname, skipValidation).map(([decl, _]) => decl);
    }

    lookupIncludeScope(qname: TsQIdent, skipValidation = false): IArray<[TsNamedDecl, TsTreeScope]> {
      return this.lookupBase(Picker.All, qname, skipValidation);
    }

    lookupType(qname: TsQIdent, skipValidation = false): IArray<TsNamedDecl> {
      return this.lookupBase(Picker.Types, qname, skipValidation).map(([decl, _]) => decl);
    }

    lookupTypeIncludeScope(qname: TsQIdent, skipValidation = false): IArray<[TsNamedDecl, TsTreeScope]> {
    ...
    private lookupBase<T extends TsNamedDecl>(
      ...
      const res = this.lookupInternal(picker, qname.parts, LoopDetector.initial);

      if (res.isEmpty && !skipValidation) {
        // For debugging - run lookup again
        this.lookupInternal(picker, qname.parts, LoopDetector.initial);
        this.logger.fatalMaybe(`Cannot resolve ${qname.asString}`, this.pedantic);
      }
    ...
    lookupInternal<T extends TsNamedDecl>(
      ...
      const stdIdent = { _tag: 'TsIdentLibrarySimple', value: 'std' } as TsIdentLibrary;
      const stdInfo = this.depScopes.get(stdIdent);
      if (stdInfo) {
        const [_, lib, libScope] = stdInfo;
        const result = this.search(libScope, picker, lib, wanted, newLoopDetector);
        if (!result.isEmpty) {
          return result;
        }
      }
      ...
      for (const [_, [lib, libFile, libScope]] of this.depScopes.entries()) {
        const result = this.search(libScope, picker, libFile, wanted, newLoopDetector);
    ...
    private search<T extends TsNamedDecl>(
      ...
      return TsTreeScopeUtils.search(scope, picker, container, fragments, loopDetector);
    }
    ...
    toString(): string {
      return `TreeScope(${this.stack.reverse().map(t => t.asString).join(' / ')})`;
    }

    equals(other: any): boolean {
      if (!(other instanceof Root)) return false;
      const that = other as Root;
      return this.libName.value === that.libName.value &&
  ...
  export class Scoped implements TsTreeScope {
    ...
    get root(): Root { return this.outer.root; }
    ...
    get parent(): TsTreeScope { return this.outer; }
    ...
    get exports(): IArray<TsExport> {
      if (!this._exports) {
        if (this.current._tag === 'TsParsedFile' ||
    ...
    private lookupBase<T extends TsNamedDecl>(
      ...
      if (res.isEmpty && !skipValidation) {
        // For debugging - run lookup again
        this.lookupInternal(picker, qname.parts, LoopDetector.initial);
        this.logger.fatalMaybe(`Cannot resolve ${qname.asString}`, this.root.pedantic);
      }
    ...
    private shouldSkip(scope: TsTreeScope): boolean {
      ...
      if (scope instanceof Scoped && scope.current._tag === 'TsParsedFile') return false;
      return true;
    }
    ...
    private lookupImpl<T extends TsNamedDecl>(
      ...
      const exportedFromModule = this.exportedFromModule(picker, wanted, loopDetector);

      // Imported from module
      const importedFromModule = this.importedFromModule(picker, wanted, loopDetector);

      // Augmented module
      const augmentedModule = this.augmentedModuleLookup(picker, wanted, loopDetector);
      ...
      if (this.current._tag === 'TsDeclClass' ||
        ...
        if (result.isEmpty && this.lookupUnqualified && !this.isDummyLibrary(wanted)) {
    ...
    private searchInContainer<T extends TsNamedDecl>(
      ...
      return TsTreeScopeUtils.search(this, picker, container, wanted, LoopDetector.initial);
    }
    ...
    private exportedFromModule<T extends TsNamedDecl>(
      ...
      if (this.current._tag === 'TsDeclNamespace' || this.current._tag === 'TsDeclModule') {
    ...
    equals(other: any): boolean {
      ...
      return this.root.libName.value === that.root.libName.value &&
             this.hashCode() === that.hashCode() &&
             this.stackEquals(that);
    }
    ...
    hashCode(): number {
      if (!this._hasHash) {
        ...
        this._hashCode = this.finalizeHash(this.mix(this.mix(2, outerHash), currentHash), 2);
      }
      return this._hashCode!;
    }
...
export class MockTsTreeScope implements TsTreeScope {
  ...
  get root(): TsTreeScope.Root {
    return TsTreeScope.create(TsIdent.librarySimple('mock'), false, new Map(), Logger.DevNull());
  }

  get lookupUnqualified(): boolean { return false; }
  get logger(): Logger<void> { return Logger.DevNull(); }
  get stack(): TsTree[] { return []; }
  get tparams(): Map<TsIdent, TsTypeParam> { return new Map(); }
  get tkeys(): Set<TsIdent> { return new Set(); }
  get parent(): TsTreeScope { return this; }
  get moduleScopes(): Map<TsIdentModule, TsTreeScope.Scoped> { return new Map(); }
  get moduleAuxScopes(): Map<TsIdentModule, TsTreeScope.Scoped> { return new Map(); }
  get exports(): IArray<TsExport> { return IArray.Empty; }
