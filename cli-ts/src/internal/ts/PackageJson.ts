...
function findLibraryInMap<V>(
	map: Map<TsIdentLibrary, V>,
	targetLib: TsIdentLibrary,
): [TsIdentLibrary, V] | undefined {
...
class PackageJson {
			...
			pipe(
				...
				O.chain((types) => {
					const result = Json.fold(
						...
						(_) => {
							throw new Error(
								`unexpected boolean in types structure: ${JSON.stringify(types)}`,
							);
						},
						(_) => {
							throw new Error(
								`unexpected number in types structure: ${JSON.stringify(types)}`,
							);
						},
						(str) => IArray.apply(str),
						(arr) =>
							IArray.fromTraversable(arr).mapNotNone((item) =>
								typeof item === "string" ? item : undefined,
							),
						(_) => {
							throw new Error(
								`unexpected object in types structure: ${JSON.stringify(types)}`,
							);
						},
					);
			...
			pipe(
				...
				O.chain((typings) => {
					const result = Json.fold(
						...
						(_) => {
							throw new Error(
								`unexpected boolean in typings structure: ${JSON.stringify(typings)}`,
							);
						},
						(_) => {
							throw new Error(
								`unexpected number in typings structure: ${JSON.stringify(typings)}`,
							);
						},
						(str) => IArray.apply(str),
						(arr) =>
						...
						(_) => {
							throw new Error(
								`unexpected object in typings structure: ${JSON.stringify(typings)}`,
							);
						},
					);
	...
	static fromObject(obj: any): PackageJson {
		const parseDependencies = (
			deps: any,
		): O.Option<Map<TsIdentLibrary, string>> => {
		...
		const parseFiles = (files: any): O.Option<IArray<string>> => {
			...
			if (Array.isArray(files)) {
				const stringFiles = files.filter(
	...
	toObject(): any {
		const serializeDependencies = (
			deps: O.Option<Map<TsIdentLibrary, string>>,
		): any => {
			return pipe(
				deps,
				O.map((depsMap) => {
		...
		return {
			...
			...(O.isSome(this._dependencies) && {
				dependencies: serializeDependencies(this._dependencies),
			}),
			...(O.isSome(this._devDependencies) && {
				devDependencies: serializeDependencies(this._devDependencies),
			}),
			...(O.isSome(this._peerDependencies) && {
				peerDependencies: serializeDependencies(this._peerDependencies),
			}),
