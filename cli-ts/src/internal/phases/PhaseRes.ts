	...
	attempt: <Id, T>(
		...
		try {
			...
			if (error instanceof Error) {
				...
				logger.error(
					`Caught non-Error exception: ${String(error)}`,
					wrappedError,
				);
...
export const map =
	<Id, T, U>(f: (value: T) => U) =>
	(phaseRes: PhaseRes<Id, T>): PhaseRes<Id, U> => {
...
export const flatMap =
	<Id, T, U>(f: (value: T) => PhaseRes<Id, U>) =>
	(phaseRes: PhaseRes<Id, T>): PhaseRes<Id, U> => {
		switch (phaseRes._tag) {
			case "Ok":
...
export const forEach =
	<Id, T>(f: (value: T) => void) =>
	(phaseRes: PhaseRes<Id, T>): PhaseRes<Id, void> => {
 ...
 */
export const isOk = <Id, T>(
	phaseRes: PhaseRes<Id, T>,
): phaseRes is { readonly _tag: "Ok"; readonly value: T } => {
	return phaseRes._tag === "Ok";
};

export const isIgnore = <Id, T>(
	phaseRes: PhaseRes<Id, T>,
): phaseRes is { readonly _tag: "Ignore" } => {
	return phaseRes._tag === "Ignore";
};

export const isFailure = <Id, T>(
	phaseRes: PhaseRes<Id, T>,
): phaseRes is {
	readonly _tag: "Failure";
	readonly errors: Map<Id, Either<Error, string>>;
