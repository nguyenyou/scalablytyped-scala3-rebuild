...
import { RecPhase, RecPhaseBase, RecPhaseInitial, RecPhaseNext } from './RecPhase';
import { PhaseListener, PhaseEvent } from './PhaseListener';
import { GetDeps, IsCircular } from './types';
...
export class PhaseRunner {
  ...
  static apply<Id, T>(
    ...
    return (initial: Id) => PhaseRunner.go(phase, initial, [], getLogger, listener, formatter, ordering);
  }
  ...
  static go<Id, TT>(
    ...
    if (phase instanceof RecPhaseInitial) {
      ...
      return PhaseRunner.doNext(phase as RecPhaseNext<Id, any, TT>, id, circuitBreaker, getLogger, listener, formatter, ordering);
    } else {
    ...
    try {
      ...
      return next.cache.getOrElse(cacheKey, () => {
        ...
        const calculateDeps: GetDeps<Id, TT> = (deps: SortedSet<Id>) => {
          if (deps.isEmpty()) {
            return PhaseRes.Ok<Id, SortedMap<Id, TT>>(new SortedMap([], ordering.compare));
          }

          listener.on(next.name, id, PhaseEvent.Blocked(next.name, new Set(deps.toArray())));
          ...
          for (const [depId, depResult] of depResults) {
            switch (depResult._tag) {
              case 'Ok':
                ...
                break;
              case 'Failure':
                depResult.errors.forEach((error, errorId) => allErrors.set(errorId, error));
                break;
              case 'Ignore':
          ...
          return PhaseRes.Ok<Id, SortedMap<Id, TT>>(
            new SortedMap(Array.from(successfulDeps.entries()), ordering.compare)
          );
        };
          ...
          switch (resLastPhase._tag) {
            case 'Ok':
              return PhaseRes.attempt(id, logger, () =>
                next.trans(id, resLastPhase.value, calculateDeps, isCircular, logger)
              );
            case 'Failure':
        ...
        switch (result._tag) {
          case 'Ok':
            ...
            break;
          case 'Failure':
            listener.on(next.name, id, PhaseEvent.Failure(next.name, result.errors));
            break;
          case 'Ignore':
      ...
      const errors = new Map<Id, any>([[id, { _tag: 'Left' as const, left: wrappedError }]]);
