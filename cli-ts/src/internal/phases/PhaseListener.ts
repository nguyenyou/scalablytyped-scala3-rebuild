...
export type PhaseEvent<Id> = 
  ...
  | { readonly _tag: 'Failure'; readonly phase: string; readonly errors: Map<Id, Either<Error, string>> }
...
export const PhaseEvent = {
   ...
   */
  Failure: <Id>(phase: string, errors: Map<Id, Either<Error, string>>): PhaseEvent<Id> => ({
    _tag: 'Failure',
    phase,
    errors
  }),
...
export class CollectingPhaseListener<Id> implements PhaseListener<Id> {
   ...
   */
  getEventsForPhase(phaseName: string): Array<{ phaseName: string; id: Id; event: PhaseEvent<Id> }> {
   ...
   */
  getEventsForId(id: Id): Array<{ phaseName: string; id: Id; event: PhaseEvent<Id> }> {
...
export class LoggingPhaseListener<Id> implements PhaseListener<Id> {
  ...
  private formatEvent(phaseName: string, id: Id, event: PhaseEvent<Id>): string {
...
export const PhaseListener = {
   ...
   */
  Collecting: <Id>(): CollectingPhaseListener<Id> => new CollectingPhaseListener<Id>(),

  /**
   * Creates a logging listener
   */
  Logging: <Id>(logLevel: 'debug' | 'info' | 'warn' | 'error' = 'info'): PhaseListener<Id> => 
    new LoggingPhaseListener<Id>(logLevel)
};

/**
 * Type guards for phase events
 */
export const isEventStarted = <Id>(event: PhaseEvent<Id>): event is { readonly _tag: 'Started'; readonly phase: string } => {
  return event._tag === 'Started';
};

export const isEventBlocked = <Id>(event: PhaseEvent<Id>): event is { readonly _tag: 'Blocked'; readonly phase: string; readonly on: Set<Id> } => {
  return event._tag === 'Blocked';
};

export const isEventSuccess = <Id>(event: PhaseEvent<Id>): event is { readonly _tag: 'Success'; readonly phase: string } => {
  return event._tag === 'Success';
};

export const isEventFailure = <Id>(event: PhaseEvent<Id>): event is { readonly _tag: 'Failure'; readonly phase: string; readonly errors: Map<Id, Either<Error, string>> } => {
  return event._tag === 'Failure';
};

export const isEventIgnored = <Id>(event: PhaseEvent<Id>): event is { readonly _tag: 'Ignored' } => {
