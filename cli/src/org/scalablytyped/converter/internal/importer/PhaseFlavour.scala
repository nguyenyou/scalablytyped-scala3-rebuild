...
import org.scalablytyped.converter.internal.scalajs.transforms.SetPrivateWithin
...
class PhaseFlavour(flavour: FlavourImpl, maybePrivateWithin: Option[Name])
  ...
  override def apply(
    ...
    getDeps((lib.dependencies.keys: Iterable[LibTsSource]).to(SortedSet)).map { deps =>
      ...
      val tree3 = maybePrivateWithin.foldLeft(tree2) { case (tree, privateWithin) =>
        new SetPrivateWithin(privateWithin).visitPackageTree(originalScope)(tree)
      }
