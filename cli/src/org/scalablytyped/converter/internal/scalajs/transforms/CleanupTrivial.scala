...
/** This is the second part of a two step process to rid ourselves of the myriad of type aliases resulting from the
  * resolution of modules.
  *
  * The first part is done in typescript, `InlineTrivialTypeAlias`, where we inline all trivial type aliases. We need to
  * do the removal here in scala.js to ensure that all dependencies can also resolve all their uses of the intermediate
  * type aliases.
  */
