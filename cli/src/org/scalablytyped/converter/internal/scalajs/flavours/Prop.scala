...
object Prop {
  ...
  /** A `Prop` can have several "variants" for providing useful overloads.
    ...
    *
    * @param asExpr
    *   takes an expression, typically the name of the parameter we generate, and rewrites it to the javascript value we
    *   want to update an object iwth
    * @param isRewritten
    *   if any useful rewrites have been done to the type or expression. Typically we filter out variants which has not
    *   been rewritten
    * @param extendsAnyVal
    *   if `tpe` is an `AnyVal` we need to make sure it's not assigned a value of `null`
    */
  case class Variant(tpe: TypeRef, asExpr: ExprTree => ExprTree, isRewritten: Boolean, extendsAnyVal: Boolean)

  /** If a type/component has too many props to fit within Javascripts 254 parameter limit, we group everything from a
    * parent type into an object of it's own.
    */
